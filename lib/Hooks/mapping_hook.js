...
exports.sw_grouping_hook = exports.hw_grouping_hook = void 0;
 ...
 */
function hw_grouping_hook(component_id, component_templates, get_hw_instances, get_sw_instances, cfg_data, secure_ctxt, genfileslist_folder, debug_print) {
    const mapping_object = []; /* the object to return the mapping rules */
    ...
    if (component_id == null ||
        ...
        if (debug === true) {
            debug_print("[ERROR] mapping_hooks.hw_grouping_hook: incorrect parameter(s)");
        }
        /* exit now */
        ...
        for (const hw_instance of my_hw_instances) {
            ...
            for (const template of component_templates) {
                ...
                mapping_object[array_index] = {
                    ...
                    resource_type: interfaces_1.RESOURCE_TYPE.HW_INSTANCE /* indicate we group by hardware instance */,
    ...
    fs.writeFile(genfileslist_folder + file_name + ".json", json_content, "utf8", function (err) {
        if (err) {
            if (debug === true) {
                debug_print(`[ERROR] mapping_hooks.hw_grouping_hook: an error occured while writing JSON Object to File.${err}`);
            }
        }
        else {
            if (debug === true) {
                debug_print("[INFO] mapping_hooks.hw_grouping_hook: " +
                    genfileslist_folder +
                    file_name +
                    ".json" +
                    " written to disk.");
...
exports.hw_grouping_hook = hw_grouping_hook;
;
/**
 ...
 * @return object describing the mapping rules to be applied, empty object if nothing to process
 */
function sw_grouping_hook(component_id, component_templates, get_hw_instances, get_sw_instances, cfg_data, secure_ctxt, genfileslist_folder, debug_print) {
    ...
    if (component_id == null ||
        ...
        genfileslist_folder == null) {
        if (debug === true) {
            debug_print("[ERROR] mapping_hooks.sw_grouping_hook: incorrect parameter(s)");
        }
        /* exit now */
        return mapping_object;
    }
...
exports.sw_grouping_hook = sw_grouping_hook;
;
//Invocation Example
//hw_grouping_hook("component_id", [], undefined ,undefined, {}, "", "", console.log);
//# sourceMappingURL=mapping_hook.js.map