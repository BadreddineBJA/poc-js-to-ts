...
exports.helper_rcc_get_clock_input = exports.helper_rcc_get_hal_clock_config_object_u5 = void 0;
function helper_rcc_get_hal_clock_config_object_u5(clock_domain) {
    if (typeof clock_domain === "undefined") {
        console.log(`[ERROR] helper_rcc_get_hal_clock_config_object_u5: clock_domain=${clock_domain}`);
        return {};
    }
    // Partial usage of the result
    let result = {
        HSI: false,
        MSI: false,
        PLL: false,
    ...
    clock_domain.forEach((element) => {
        ...
        if (id == "HSI") {
            result.HSI = true;
            if (result.OscillatorType != "")
                result.OscillatorType += " | ";
...
exports.helper_rcc_get_hal_clock_config_object_u5 = helper_rcc_get_hal_clock_config_object_u5;
function helper_rcc_get_clock_input(clock_domain, hw_resource) {
    let result = {
        found: false,
    ...
    console.log('clocksource', clock_source);
    clock_domain.forEach((clock) => {
        if (clock.id == clock_source) {
            if (input_map_hal.hasOwnProperty(clock.value) &&
                ...
                console.log(`[ERROR] helper_rcc_get_clock_input: input or hw_resource not possible`);
            }
            return;
        }
    });
...
exports.helper_rcc_get_clock_input = helper_rcc_get_clock_input;
//# sourceMappingURL=rcc_helpers.js.map