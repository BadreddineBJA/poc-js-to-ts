...
import { GPIOConfig, GpioList2 } from "./interfaces";
...
export function helper_exti_get_gpio(Gpio_Config: GPIOConfig): GPIO_RESULT {
  console.log(`[INFO] helper_exti_get_gpio`);
  var groups = Gpio_Config["group"];
  var result: GPIO_RESULT = { found_exti: false, exti: [], label_exti: [] };
  groups.forEach((group) => {
    ...
    configs.forEach((config) => {
      ...
      gpio_list.forEach((list: GpioList2, index) => {
        var pin_label = list["label"];
        if (list["exti"]) {
          var exti_line: number = gpio_list_hw[index]["gpio"]["pin"];
          var labels = {
            exti: exti_line,
            grp_name: grp_name,
            grp_label: grp_label,
...
export function helper_exti_get_config(gpio_list: any) {
  ...
  let result: CONFIG_RESULT = { found_exti: false, exti_list: [] };

  gpio_list.forEach((gpio: any) => {
    let gpio_hw = gpio_list["pad"];
    let list_exti: LIST_EXTI = { port: "", line: "", trigger: "" };
    let exti = gpio["exti"];
    if (exti) {
      result["found_exti"] = true;
