...
export function helper_rcc_get_hal_clock_config_object_u5(
  clock_domain: ClockConfig[]
): Partial<IClockHALConfig> {
  ...
  let result: Partial<IClockHALConfig> = {
    HSI: false,
    MSI: false,
    PLL: false,
  ...
  clock_domain.forEach((element: ClockConfig) => {
    let id = element.id;
    ...
    if (id == "MSIS") {
      ...
      const map_msis_range: { [key: number]: string } = {
    ...
    if (id == "PLL1_Clock_Source") {
      result.PLL = true;
      const map_pll_clk_src: { [key: string]: string } = {
    ...
    if (id == "System_Clock_Source") {
      const map_sys_clk_src: { [key: string]: string } = {
...
export function helper_rcc_get_clock_input(
  clock_domain: ClockConfig[],
  hw_resource: string
): IClockInputResult {
  let result: IClockInputResult = {
    found: false,
    clockselection: "Usart1ClockSelection",
    input: "RCC_USART1CLKSOURCE_PCLK2",
  };
  const input_map_hal: { [key: string]: string } = {
  ...
  const resource_map_clk_selection: { [key: string]: string } = {
