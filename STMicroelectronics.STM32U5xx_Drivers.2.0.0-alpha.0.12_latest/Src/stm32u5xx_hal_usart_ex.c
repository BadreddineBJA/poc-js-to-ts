...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_EnableSlaveMode(USART_HandleTypeDef *husart)
{
  uint32_t tmpcr1;

  /* Check parameters */
  ASSERT_DBG_PARAM(IS_UART_SPI_SLAVE_INSTANCE(husart->Instance));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_DisableSlaveMode(USART_HandleTypeDef *husart)
{
  uint32_t tmpcr1;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_ConfigNSS(USART_HandleTypeDef *husart, uint32_t NSSConfig)
{
  uint32_t tmpcr1;

  /* Check parameters */
  ASSERT_DBG_PARAM(IS_UART_SPI_SLAVE_INSTANCE(husart->Instance));
  ASSERT_DBG_PARAM(IS_USART_NSS(NSSConfig));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_EnableFifoMode(USART_HandleTypeDef *husart)
{
  uint32_t tmpcr1;

  /* Check parameters */
  ASSERT_DBG_PARAM(IS_UART_FIFO_INSTANCE(husart->Instance));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_DisableFifoMode(USART_HandleTypeDef *husart)
{
  uint32_t tmpcr1;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_SetTxFifoThreshold(USART_HandleTypeDef *husart, uint32_t Threshold)
{
  uint32_t tmpcr1;

  /* Check parameters */
  ASSERT_DBG_PARAM(IS_UART_FIFO_INSTANCE(husart->Instance));
  ASSERT_DBG_PARAM(IS_USART_TXFIFO_THRESHOLD(Threshold));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_SetRxFifoThreshold(USART_HandleTypeDef *husart, uint32_t Threshold)
{
  uint32_t tmpcr1;

  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_UART_FIFO_INSTANCE(husart->Instance));
  ASSERT_DBG_PARAM(IS_USART_RXFIFO_THRESHOLD(Threshold));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_SetConfigAutonomousMode(USART_HandleTypeDef *husart,
                                                 const USART_AutonomousModeConfTypeDef *sConfig)
{
  ...
  {
    /* Check the parameters */
    ASSERT_DBG_PARAM(IS_USART_TRIGGER_POLARITY(sConfig->TriggerPolarity));
    ASSERT_DBG_PARAM(IS_USART_IDLE_FRAME_TRANSMIT(sConfig->IdleFrame));
    ASSERT_DBG_PARAM(IS_USART_TX_DATA_SIZE(sConfig->DataSize));
    ASSERT_DBG_PARAM(IS_USART_TRIGGER_SELECTION(sConfig->TriggerSelection));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_GetConfigAutonomousMode(const USART_HandleTypeDef *husart,
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_USARTEx_ClearConfigAutonomousMode(USART_HandleTypeDef *husart)
{
  if (husart->State == HAL_USART_STATE_READY)
  {
    /* Process Locked */
