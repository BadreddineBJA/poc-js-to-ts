...
static hal_status_t OSPI_WaitFlagStateUntilTimeout(OSPI_HandleTypeDef *hospi, uint32_t Flag, FlagStatus State,
                                                   uint32_t Tickstart, uint32_t Timeout);
static hal_status_t OSPI_ConfigCmd(OSPI_HandleTypeDef *hospi, OSPI_RegularCmdTypeDef *cmd);
static hal_status_t OSPIM_GetConfig(uint8_t instance_nb, OSPIM_CfgTypeDef *cfg);
/**
  @endcond
  */
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Init(OSPI_HandleTypeDef *hospi)
{
  ...
  else
  {
    ...
    ASSERT_DBG_PARAM(IS_OSPI_FIFO_THRESHOLD(hospi->Init.FifoThreshold));
    ASSERT_DBG_PARAM(IS_OSPI_DUALQUAD_MODE(hospi->Init.DualQuad));
    ASSERT_DBG_PARAM(IS_OSPI_MEMORY_TYPE(hospi->Init.MemoryType));
    ASSERT_DBG_PARAM(IS_OSPI_DEVICE_SIZE(hospi->Init.DeviceSize));
    ASSERT_DBG_PARAM(IS_OSPI_CS_HIGH_TIME(hospi->Init.ChipSelectHighTime));
    ASSERT_DBG_PARAM(IS_OSPI_FREE_RUN_CLK(hospi->Init.FreeRunningClock));
    ASSERT_DBG_PARAM(IS_OSPI_CLOCK_MODE(hospi->Init.ClockMode));
    ASSERT_DBG_PARAM(IS_OSPI_WRAP_SIZE(hospi->Init.WrapSize));
    ASSERT_DBG_PARAM(IS_OSPI_CLK_PRESCALER(hospi->Init.ClockPrescaler));
    ASSERT_DBG_PARAM(IS_OSPI_SAMPLE_SHIFTING(hospi->Init.SampleShifting));
    ASSERT_DBG_PARAM(IS_OSPI_DHQC(hospi->Init.DelayHoldQuarterCycle));
    ASSERT_DBG_PARAM(IS_OSPI_CS_BOUNDARY(hospi->Init.ChipSelectBoundary));
    ASSERT_DBG_PARAM(IS_OSPI_DLYBYP(hospi->Init.DelayBlockBypass));
    ASSERT_DBG_PARAM(IS_OSPI_MAXTRAN(hospi->Init.MaxTran));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_DeInit(OSPI_HandleTypeDef *hospi)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Command(OSPI_HandleTypeDef *hospi, OSPI_RegularCmdTypeDef *cmd, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t state;
  uint32_t tickstart = HAL_GetTick();
  ...
  ASSERT_DBG_PARAM(IS_OSPI_OPERATION_TYPE(cmd->OperationType));

  if (hospi->Init.DualQuad == HAL_OSPI_DUALQUAD_DISABLE)
  {
    ASSERT_DBG_PARAM(IS_OSPI_FLASH_ID(cmd->FlashId));
  }

  ASSERT_DBG_PARAM(IS_OSPI_INSTRUCTION_MODE(cmd->InstructionMode));
  if (cmd->InstructionMode != HAL_OSPI_INSTRUCTION_NONE)
  {
    ASSERT_DBG_PARAM(IS_OSPI_INSTRUCTION_SIZE(cmd->InstructionSize));
    ASSERT_DBG_PARAM(IS_OSPI_INSTRUCTION_DTR_MODE(cmd->InstructionDtrMode));
  }

  ASSERT_DBG_PARAM(IS_OSPI_ADDRESS_MODE(cmd->AddressMode));
  if (cmd->AddressMode != HAL_OSPI_ADDRESS_NONE)
  {
    ASSERT_DBG_PARAM(IS_OSPI_ADDRESS_SIZE(cmd->AddressSize));
    ASSERT_DBG_PARAM(IS_OSPI_ADDRESS_DTR_MODE(cmd->AddressDtrMode));
  }

  ASSERT_DBG_PARAM(IS_OSPI_ALT_BYTES_MODE(cmd->AlternateBytesMode));
  if (cmd->AlternateBytesMode != HAL_OSPI_ALTERNATE_BYTES_NONE)
  {
    ASSERT_DBG_PARAM(IS_OSPI_ALT_BYTES_SIZE(cmd->AlternateBytesSize));
    ASSERT_DBG_PARAM(IS_OSPI_ALT_BYTES_DTR_MODE(cmd->AlternateBytesDtrMode));
  }

  ASSERT_DBG_PARAM(IS_OSPI_DATA_MODE(cmd->DataMode));
  if (cmd->DataMode != HAL_OSPI_DATA_NONE)
  {
    if (cmd->OperationType == HAL_OSPI_OPTYPE_COMMON_CFG)
    {
      ASSERT_DBG_PARAM(IS_OSPI_NUMBER_DATA(cmd->NbData));
    }
    ASSERT_DBG_PARAM(IS_OSPI_DATA_DTR_MODE(cmd->DataDtrMode));
    ASSERT_DBG_PARAM(IS_OSPI_DUMMY_CYCLES(cmd->DummyCycles));
  }

  ASSERT_DBG_PARAM(IS_OSPI_DQS_MODE(cmd->DQSMode));
  ASSERT_DBG_PARAM(IS_OSPI_SIOO_MODE(cmd->SIOOMode));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Command_IT(OSPI_HandleTypeDef *hospi, OSPI_RegularCmdTypeDef *cmd)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_HyperbusCfg(OSPI_HandleTypeDef *hospi, OSPI_HyperbusCfgTypeDef *cfg, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t state;
  uint32_t tickstart = HAL_GetTick();
  ...
  ASSERT_DBG_PARAM(IS_OSPI_RW_RECOVERY_TIME(cfg->RWRecoveryTime));
  ASSERT_DBG_PARAM(IS_OSPI_ACCESS_TIME(cfg->AccessTime));
  ASSERT_DBG_PARAM(IS_OSPI_WRITE_ZERO_LATENCY(cfg->WriteZeroLatency));
  ASSERT_DBG_PARAM(IS_OSPI_LATENCY_MODE(cfg->LatencyMode));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_HyperbusCmd(OSPI_HandleTypeDef *hospi, OSPI_HyperbusCmdTypeDef *cmd, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();

  /* Check the parameters of the hyperbus command structure */
  ASSERT_DBG_PARAM(IS_OSPI_ADDRESS_SPACE(cmd->AddressSpace));
  ASSERT_DBG_PARAM(IS_OSPI_ADDRESS_SIZE(cmd->AddressSize));
  ASSERT_DBG_PARAM(IS_OSPI_NUMBER_DATA(cmd->NbData));
  ASSERT_DBG_PARAM(IS_OSPI_DQS_MODE(cmd->DQSMode));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Transmit(OSPI_HandleTypeDef *hospi, uint8_t *pData, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  __IO uint32_t *data_reg = &hospi->Instance->DR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Receive(OSPI_HandleTypeDef *hospi, uint8_t *pData, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  __IO uint32_t *data_reg = &hospi->Instance->DR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Transmit_IT(OSPI_HandleTypeDef *hospi, uint8_t *pData)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Receive_IT(OSPI_HandleTypeDef *hospi, uint8_t *pData)
{
  hal_status_t status = HAL_OK;
  uint32_t addr_reg = hospi->Instance->AR;
  uint32_t ir_reg = hospi->Instance->IR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Transmit_DMA(OSPI_HandleTypeDef *hospi, uint8_t *pData)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Receive_DMA(OSPI_HandleTypeDef *hospi, uint8_t *pData)
{
  hal_status_t status = HAL_OK;
  uint32_t data_size = hospi->Instance->DLR + 1U;
  uint32_t addr_reg = hospi->Instance->AR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_AutoPolling(OSPI_HandleTypeDef *hospi, OSPI_AutoPollingTypeDef *cfg, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  uint32_t addr_reg = hospi->Instance->AR;
  ...
  ASSERT_DBG_PARAM(IS_OSPI_MATCH_MODE(cfg->MatchMode));
  ASSERT_DBG_PARAM(IS_OSPI_AUTOMATIC_STOP(cfg->AutomaticStop));
  ASSERT_DBG_PARAM(IS_OSPI_INTERVAL(cfg->Interval));
  ASSERT_DBG_PARAM(IS_OSPI_STATUS_BYTES_SIZE(dlr_reg + 1U));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_AutoPolling_IT(OSPI_HandleTypeDef *hospi, OSPI_AutoPollingTypeDef *cfg)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  uint32_t addr_reg = hospi->Instance->AR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_MemoryMapped(OSPI_HandleTypeDef *hospi, OSPI_MemoryMappedTypeDef *cfg)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();

  /* Check the parameters of the memory-mapped configuration structure */
  ASSERT_DBG_PARAM(IS_OSPI_TIMEOUT_ACTIVATION(cfg->TimeOutActivation));
  ...
  if (hospi->State == HAL_OSPI_STATE_CMD_CFG)
  {
    ...
    if (status == HAL_OK)
    {
      ...
      {
        ASSERT_DBG_PARAM(IS_OSPI_TIMEOUT_PERIOD(cfg->TimeOutPeriod));
...
/**
  ...
  * @retval status
  */
hal_status_t HAL_OSPI_RegisterCallback(OSPI_HandleTypeDef *hospi, HAL_OSPI_CallbackIDTypeDef CallbackID,
...
/**
  ...
  * @retval status
  */
hal_status_t HAL_OSPI_UnRegisterCallback(OSPI_HandleTypeDef *hospi, HAL_OSPI_CallbackIDTypeDef CallbackID)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Abort(OSPI_HandleTypeDef *hospi)
{
  hal_status_t status = HAL_OK;
  uint32_t state;
  uint32_t tickstart = HAL_GetTick();
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_Abort_IT(OSPI_HandleTypeDef *hospi)
{
  hal_status_t status = HAL_OK;
  uint32_t state;
...
/** @brief  Set OSPI Fifo threshold.
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPI_SetFifoThreshold(OSPI_HandleTypeDef *hospi, uint32_t Threshold)
{
  hal_status_t status = HAL_OK;
...
/** @brief Set OSPI timeout.
  ...
  * @retval None
  */
hal_status_t HAL_OSPI_SetTimeout(OSPI_HandleTypeDef *hospi, uint32_t Timeout)
{
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_OSPIM_Config(OSPI_HandleTypeDef *hospi, OSPIM_CfgTypeDef *cfg, uint32_t Timeout)
{
  hal_status_t status = HAL_OK;
  uint32_t instance;
  uint8_t index;
  ...
  ASSERT_DBG_PARAM(IS_OSPIM_PORT(cfg->ClkPort));
  ASSERT_DBG_PARAM(IS_OSPIM_DQS_PORT(cfg->DQSPort));
  ASSERT_DBG_PARAM(IS_OSPIM_PORT(cfg->NCSPort));
  ASSERT_DBG_PARAM(IS_OSPIM_IO_PORT(cfg->IOLowPort));
  ASSERT_DBG_PARAM(IS_OSPIM_IO_PORT(cfg->IOHighPort));
...
/**
  ...
  * @retval HAL status
  */
static hal_status_t OSPI_WaitFlagStateUntilTimeout(OSPI_HandleTypeDef *hospi, uint32_t Flag,
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_OSPI_DLYB_SetConfig(OSPI_HandleTypeDef *hospi, HAL_OSPI_DLYB_CfgTypeDef  *pdlyb_cfg)
{
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_OSPI_DLYB_GetConfig(OSPI_HandleTypeDef *hospi, HAL_OSPI_DLYB_CfgTypeDef  *pdlyb_cfg)
{
  hal_status_t status = HAL_ERROR;
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_OSPI_DLYB_GetClockPeriod(OSPI_HandleTypeDef *hospi, HAL_OSPI_DLYB_CfgTypeDef  *pdlyb_cfg)
{
  hal_status_t status = HAL_ERROR;
