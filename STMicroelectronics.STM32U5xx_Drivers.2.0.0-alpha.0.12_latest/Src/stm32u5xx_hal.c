...
/** @defgroup HAL_Private_Defines HAL Private Defines
  ...
  */
/**
  * @brief STM32U5xx HAL Driver version number 2.0.0
   */
#define __STM32U5xx_HAL_VERSION_MAIN   (0x02U) /*!< [31:24] main version */
...
/**
  ...
  * @retval HAL status
  */
__weak hal_status_t HAL_InitTick(uint32_t TickPriority)
{
...
hal_status_t HAL_SetTickFreq(HAL_TickFreqTypeDef Freq)
{
  ...
  ASSERT_DBG_PARAM(IS_TICKFREQ(Freq));
...
void HAL_SYSCFG_VREFBUF_VoltageScalingConfig(uint32_t VoltageScaling)
{
  ...
  ASSERT_DBG_PARAM(IS_SYSCFG_VREFBUF_VOLTAGE_SCALE(VoltageScaling));
...
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SYSCFG_VREFBUF_HIGH_IMPEDANCE(Mode));
...
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SYSCFG_VREFBUF_TRIMMING(TrimmingValue));
...
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SYSCFG_LOCK_ITEMS(Item));
...
void HAL_SYSCFG_ConfigAttributes(uint32_t Item, uint32_t Attributes)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SYSCFG_ITEMS_ATTRIBUTES(Item));
  ASSERT_DBG_PARAM(IS_SYSCFG_ATTRIBUTES(Attributes));
...
{
  /* Check the parameter */
  ASSERT_DBG_PARAM(IS_SYSCFG_OTGPHY_CONFIG(OTGPHYConfig));
...
{
  /* Check the parameter */
  ASSERT_DBG_PARAM(IS_SYSCFG_OTGPHY_POWERDOWN_CONFIG(PowerDownConfig));
...
{
  /* Check the parameter */
  ASSERT_DBG_PARAM(IS_SYSCFG_OTGPHY_REFERENCE_CLOCK(RefClockSelection));
