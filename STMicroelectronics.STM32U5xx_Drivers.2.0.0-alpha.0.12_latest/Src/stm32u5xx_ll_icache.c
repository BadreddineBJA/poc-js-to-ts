...
void LL_ICACHE_ConfigRegion(uint32_t Region, const LL_ICACHE_RegionTypeDef *const pICACHE_RegionStruct)
{
  ...
  ASSERT_DBG_PARAM(IS_LL_ICACHE_REGION(Region));
  ASSERT_DBG_PARAM(IS_LL_ICACHE_REGION_SIZE(pICACHE_RegionStruct->Size));
  ASSERT_DBG_PARAM(IS_LL_ICACHE_MASTER_PORT(pICACHE_RegionStruct->TrafficRoute));
  ASSERT_DBG_PARAM(IS_LL_ICACHE_OUTPUT_BURST(pICACHE_RegionStruct->OutputBurstType));
