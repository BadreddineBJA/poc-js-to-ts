...
hal_status_t HAL_SMBUSEx_EnableWakeUp(SMBUS_HandleTypeDef *hsmbus)
{
  ...
  ASSERT_DBG_PARAM(IS_I2C_WAKEUP_FROMSTOP_INSTANCE(hsmbus->Instance));
...
hal_status_t HAL_SMBUSEx_ConfigFastModePlus(SMBUS_HandleTypeDef *hsmbus, uint32_t FastModePlus)
{
  ...
  ASSERT_DBG_PARAM(IS_SMBUS_FASTMODEPLUS(FastModePlus));
  ...
  if (hsmbus->State == HAL_SMBUS_STATE_READY)
  {
    ...
    if (FastModePlus == HAL_SMBUS_FASTMODEPLUS_ENABLE)
    {
      /* Set SMBUSx FMP bit */
                                                 ...
                                                 SMBUS_AutonomousModeConfTypeDef *sConfig)
{
  if (hsmbus->State == HAL_SMBUS_STATE_READY)
  {
    ...
    /* Check the parameters */
    ASSERT_DBG_PARAM(IS_SMBUS_TRIG_SOURCE(hsmbus->Instance, sConfig->TriggerSelection));

    ASSERT_DBG_PARAM(IS_SMBUS_AUTO_MODE_TRG_POL(sConfig->TriggerPolarity));
                                                 ...
                                                 SMBUS_AutonomousModeConfTypeDef *sConfig)
{
  ...
  {
    sConfig->TriggerSelection = ((autocr_tmp & I2C_AUTOCR_TRIGSEL) | HAL_SMBUS_TRIG_GRP2);
  }
  else
  {
    sConfig->TriggerSelection = ((autocr_tmp & I2C_AUTOCR_TRIGSEL) | HAL_SMBUS_TRIG_GRP1);
  }
