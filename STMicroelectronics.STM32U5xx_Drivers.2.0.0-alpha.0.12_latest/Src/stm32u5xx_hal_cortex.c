...
void HAL_NVIC_SetPriorityGrouping(uint32_t PriorityGroup)
{
  ...
  ASSERT_DBG_PARAM(IS_NVIC_PRIORITY_GROUP(PriorityGroup));
...
void HAL_NVIC_SetPriority(IRQn_Type IRQn, uint32_t PreemptPriority, uint32_t SubPriority)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_NVIC_SUB_PRIORITY(SubPriority));
  ASSERT_DBG_PARAM(IS_NVIC_PREEMPTION_PRIORITY(PreemptPriority));
...
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_NVIC_DEVICE_IRQ(IRQn));
...
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SYSTICK_CLK_SOURCE(CLKSource));
  switch (CLKSource)
  {
...
static void MPU_ConfigRegion(MPU_Type *MPUx, const MPU_Region_InitTypeDef *const pMPU_RegionInit)
{
  /* Check the parameters */
#if defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
  ASSERT_DBG_PARAM(IS_MPU_INSTANCE(MPUx));
#endif /* __ARM_FEATURE_CMSE */
  ASSERT_DBG_PARAM(IS_MPU_REGION_NUMBER(pMPU_RegionInit->Number));
  ASSERT_DBG_PARAM(IS_MPU_REGION_ENABLE(pMPU_RegionInit->Enable));
  ...
  {
    /* Check the parameters */
    ASSERT_DBG_PARAM(IS_MPU_INSTRUCTION_ACCESS(pMPU_RegionInit->DisableExec));
    ASSERT_DBG_PARAM(IS_MPU_REGION_PERMISSION_ATTRIBUTE(pMPU_RegionInit->AccessPermission));
    ASSERT_DBG_PARAM(IS_MPU_ACCESS_SHAREABLE(pMPU_RegionInit->IsShareable));
...
static void MPU_ConfigMemoryAttributes(MPU_Type *MPUx, const MPU_Attributes_InitTypeDef *const pMPU_AttributesInit)
{
  ...
  ASSERT_DBG_PARAM(IS_MPU_INSTANCE(MPUx));
#endif /* __ARM_FEATURE_CMSE */
  ASSERT_DBG_PARAM(IS_MPU_ATTRIBUTES_NUMBER(pMPU_AttributesInit->Number));
