...
uint32_t LL_DMA_DeInit(dma_t *DMAx, uint32_t Channel)
{
  ...
  ASSERT_DBG_PARAM(IS_LL_DMA_ALL_CHANNEL_INSTANCE(DMAx, Channel));
  ...
  else
  {
    ...
    tmp = (dma_channel_t *)(LL_DMA_GET_CHANNEL_INSTANCE(DMAx, Channel));
...
uint32_t LL_DMA_Init(dma_t *DMAx, uint32_t Channel, LL_DMA_InitTypeDef *DMA_InitStruct)
{
  ...
  ASSERT_DBG_PARAM(IS_LL_DMA_DIRECTION(DMA_InitStruct->Direction));

  /* Check direction */
  if (DMA_InitStruct->Direction != LL_DMA_DIRECTION_MEMORY_TO_MEMORY)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_REQUEST_SELECTION(DMA_InitStruct->Request));
  }

  ASSERT_DBG_PARAM(IS_LL_DMA_DATA_ALIGNMENT(DMA_InitStruct->DataAlignment));
  ASSERT_DBG_PARAM(IS_LL_DMA_SRC_DATA_WIDTH(DMA_InitStruct->SrcDataWidth));
  ASSERT_DBG_PARAM(IS_LL_DMA_DEST_DATA_WIDTH(DMA_InitStruct->DestDataWidth));
  ASSERT_DBG_PARAM(IS_LL_DMA_SRC_INCREMENT_MODE(DMA_InitStruct->SrcIncMode));
  ASSERT_DBG_PARAM(IS_LL_DMA_DEST_INCREMENT_MODE(DMA_InitStruct->DestIncMode));
  ASSERT_DBG_PARAM(IS_LL_DMA_PRIORITY(DMA_InitStruct->Priority));
  ASSERT_DBG_PARAM(IS_LL_DMA_BLK_DATALENGTH(DMA_InitStruct->BlkDataLength));
  ASSERT_DBG_PARAM(IS_LL_DMA_TRIGGER_POLARITY(DMA_InitStruct->TriggerPolarity));
  ASSERT_DBG_PARAM(IS_LL_DMA_BLKHW_REQUEST(DMA_InitStruct->BlkHWRequest));
  ASSERT_DBG_PARAM(IS_LL_DMA_TRANSFER_EVENT_MODE(DMA_InitStruct->TransferEventMode));
  ASSERT_DBG_PARAM(IS_LL_DMA_LINK_STEP_MODE(DMA_InitStruct->LinkStepMode));
  ASSERT_DBG_PARAM(IS_LL_DMA_LINK_BASEADDR(DMA_InitStruct->LinkedListBaseAddr));
  ASSERT_DBG_PARAM(IS_LL_DMA_LINK_ADDR_OFFSET(DMA_InitStruct->LinkedListAddrOffset));

  /* Check DMA instance */
  if (IS_LL_GPDMA_CHANNEL_INSTANCE(DMAx, Channel) != 0U)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_LENGTH(DMA_InitStruct->SrcBurstLength));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_LENGTH(DMA_InitStruct->DestBurstLength));
    ASSERT_DBG_PARAM(IS_LL_DMA_DEST_HALFWORD_EXCHANGE(DMA_InitStruct->DestHWordExchange));
    ASSERT_DBG_PARAM(IS_LL_DMA_DEST_BYTE_EXCHANGE(DMA_InitStruct->DestByteExchange));
    ASSERT_DBG_PARAM(IS_LL_DMA_SRC_BYTE_EXCHANGE(DMA_InitStruct->SrcByteExchange));
    ASSERT_DBG_PARAM(IS_LL_DMA_LINK_ALLOCATED_PORT(DMA_InitStruct->LinkAllocatedPort));
    ASSERT_DBG_PARAM(IS_LL_DMA_SRC_ALLOCATED_PORT(DMA_InitStruct->SrcAllocatedPort));
    ASSERT_DBG_PARAM(IS_LL_DMA_DEST_ALLOCATED_PORT(DMA_InitStruct->DestAllocatedPort));
  }

  /* Check trigger polarity */
  if (DMA_InitStruct->TriggerPolarity != LL_DMA_TRIG_POLARITY_MASKED)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_TRIGGER_MODE(DMA_InitStruct->TriggerMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_TRIGGER_SELECTION(DMA_InitStruct->TriggerSelection));
  }

  /* Check DMA channel */
  if (IS_LL_DMA_2D_CHANNEL_INSTANCE(DMAx, Channel) != 0U)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_BLK_REPEATCOUNT(DMA_InitStruct->BlkRptCount));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_SRC_ADDR_UPDATE(DMA_InitStruct->SrcAddrUpdateMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_DEST_ADDR_UPDATE(DMA_InitStruct->DestAddrUpdateMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_ADDR_UPDATE_VALUE(DMA_InitStruct->SrcAddrOffset));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_ADDR_UPDATE_VALUE(DMA_InitStruct->DestAddrOffset));
    ASSERT_DBG_PARAM(IS_LL_DMA_BLKRPT_SRC_ADDR_UPDATE(DMA_InitStruct->BlkRptSrcAddrUpdateMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_BLKRPT_DEST_ADDR_UPDATE(DMA_InitStruct->BlkRptDestAddrUpdateMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_BLKRPT_ADDR_UPDATE_VALUE(DMA_InitStruct->BlkRptSrcAddrOffset));
    ASSERT_DBG_PARAM(IS_LL_DMA_BLKRPT_ADDR_UPDATE_VALUE(DMA_InitStruct->BlkRptDestAddrOffset));
  }
...
/**
  ...
  *          - ERROR   : Not applicable.
  */
uint32_t LL_DMA_List_Init(dma_t *DMAx, uint32_t Channel, LL_DMA_InitLinkedListTypeDef *DMA_InitLinkedListStruct)
{
  /* Check the DMA Instance DMAx and Channel parameters*/
  ASSERT_DBG_PARAM(IS_LL_DMA_ALL_CHANNEL_INSTANCE(DMAx, Channel));

  /* Check the DMA parameters from DMA_InitLinkedListStruct */
  ASSERT_DBG_PARAM(IS_LL_DMA_PRIORITY(DMA_InitLinkedListStruct->Priority));
  ASSERT_DBG_PARAM(IS_LL_DMA_LINK_STEP_MODE(DMA_InitLinkedListStruct->LinkStepMode));
  ASSERT_DBG_PARAM(IS_LL_DMA_TRANSFER_EVENT_MODE(DMA_InitLinkedListStruct->TransferEventMode));
  /* Check DMA instance */
  if (IS_LL_GPDMA_CHANNEL_INSTANCE(DMAx, Channel) != 0U)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_LINK_ALLOCATED_PORT(DMA_InitLinkedListStruct->LinkAllocatedPort));
  }
...
uint32_t LL_DMA_CreateLinkNode(LL_DMA_InitNodeTypeDef *DMA_InitNodeStruct, LL_DMA_LinkNodeTypeDef *pNode)
{
  ...
  ASSERT_DBG_PARAM(IS_LL_DMA_LINK_NODETYPE(DMA_InitNodeStruct->NodeType));

  /* Check the DMA parameters from DMA_InitNodeStruct */
  ASSERT_DBG_PARAM(IS_LL_DMA_DIRECTION(DMA_InitNodeStruct->Direction));

  /* Check direction */
  if (DMA_InitNodeStruct->Direction != LL_DMA_DIRECTION_MEMORY_TO_MEMORY)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_REQUEST_SELECTION(DMA_InitNodeStruct->Request));
  }

  ASSERT_DBG_PARAM(IS_LL_DMA_DATA_ALIGNMENT(DMA_InitNodeStruct->DataAlignment));
  ASSERT_DBG_PARAM(IS_LL_DMA_SRC_DATA_WIDTH(DMA_InitNodeStruct->SrcDataWidth));
  ASSERT_DBG_PARAM(IS_LL_DMA_DEST_DATA_WIDTH(DMA_InitNodeStruct->DestDataWidth));
  ASSERT_DBG_PARAM(IS_LL_DMA_SRC_INCREMENT_MODE(DMA_InitNodeStruct->SrcIncMode));
  ASSERT_DBG_PARAM(IS_LL_DMA_DEST_INCREMENT_MODE(DMA_InitNodeStruct->DestIncMode));
  ASSERT_DBG_PARAM(IS_LL_DMA_BLK_DATALENGTH(DMA_InitNodeStruct->BlkDataLength));
  ASSERT_DBG_PARAM(IS_LL_DMA_TRIGGER_POLARITY(DMA_InitNodeStruct->TriggerPolarity));
  ASSERT_DBG_PARAM(IS_LL_DMA_BLKHW_REQUEST(DMA_InitNodeStruct->BlkHWRequest));
  ASSERT_DBG_PARAM(IS_LL_DMA_TRANSFER_EVENT_MODE(DMA_InitNodeStruct->TransferEventMode));
  ASSERT_DBG_PARAM(IS_LL_DMA_LINK_UPDATE_REGISTERS(DMA_InitNodeStruct->UpdateRegisters));

#if defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U)
  ASSERT_DBG_PARAM(IS_LL_DMA_CHANNEL_SRC_SEC(DMA_InitNodeStruct->SrcSecure));
  ASSERT_DBG_PARAM(IS_LL_DMA_CHANNEL_DEST_SEC(DMA_InitNodeStruct->DestSecure));
#endif /* defined (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U) */

  /* Check trigger polarity */
  if (DMA_InitNodeStruct->TriggerPolarity != LL_DMA_TRIG_POLARITY_MASKED)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_TRIGGER_MODE(DMA_InitNodeStruct->TriggerMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_TRIGGER_SELECTION(DMA_InitNodeStruct->TriggerSelection));
  }

  /* Check node type */
  if (DMA_InitNodeStruct->NodeType == LL_DMA_GPDMA_LINEAR_NODE)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_LENGTH(DMA_InitNodeStruct->SrcBurstLength));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_LENGTH(DMA_InitNodeStruct->DestBurstLength));
    ASSERT_DBG_PARAM(IS_LL_DMA_DEST_HALFWORD_EXCHANGE(DMA_InitNodeStruct->DestHWordExchange));
    ASSERT_DBG_PARAM(IS_LL_DMA_DEST_BYTE_EXCHANGE(DMA_InitNodeStruct->DestByteExchange));
    ASSERT_DBG_PARAM(IS_LL_DMA_SRC_BYTE_EXCHANGE(DMA_InitNodeStruct->SrcByteExchange));
    ASSERT_DBG_PARAM(IS_LL_DMA_SRC_ALLOCATED_PORT(DMA_InitNodeStruct->SrcAllocatedPort));
    ASSERT_DBG_PARAM(IS_LL_DMA_DEST_ALLOCATED_PORT(DMA_InitNodeStruct->DestAllocatedPort));
  }

  /* Check DMA channel */
  if (DMA_InitNodeStruct->NodeType == LL_DMA_GPDMA_2D_NODE)
  {
    ASSERT_DBG_PARAM(IS_LL_DMA_BLK_REPEATCOUNT(DMA_InitNodeStruct->BlkRptCount));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_SRC_ADDR_UPDATE(DMA_InitNodeStruct->SrcAddrUpdateMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_DEST_ADDR_UPDATE(DMA_InitNodeStruct->DestAddrUpdateMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_ADDR_UPDATE_VALUE(DMA_InitNodeStruct->SrcAddrOffset));
    ASSERT_DBG_PARAM(IS_LL_DMA_BURST_ADDR_UPDATE_VALUE(DMA_InitNodeStruct->DestAddrOffset));
    ASSERT_DBG_PARAM(IS_LL_DMA_BLKRPT_SRC_ADDR_UPDATE(DMA_InitNodeStruct->BlkRptSrcAddrUpdateMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_BLKRPT_DEST_ADDR_UPDATE(DMA_InitNodeStruct->BlkRptDestAddrUpdateMode));
    ASSERT_DBG_PARAM(IS_LL_DMA_BLKRPT_ADDR_UPDATE_VALUE(DMA_InitNodeStruct->BlkRptSrcAddrOffset));
    ASSERT_DBG_PARAM(IS_LL_DMA_BLKRPT_ADDR_UPDATE_VALUE(DMA_InitNodeStruct->BlkRptDestAddrOffset));
  }
