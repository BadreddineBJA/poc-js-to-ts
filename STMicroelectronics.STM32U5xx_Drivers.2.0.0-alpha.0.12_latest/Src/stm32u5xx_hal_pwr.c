...
/**
  ...
  * @retval HAL Status.
  */
hal_status_t HAL_PWR_ConfigPVD(PWR_PVDTypeDef *pConfigPVD)
{
  ...
  ASSERT_DBG_PARAM(IS_PWR_PVD_LEVEL(pConfigPVD->PVDLevel));
  ASSERT_DBG_PARAM(IS_PWR_PVD_MODE(pConfigPVD->Mode));
...
{
  /* Check the parameter */
  ASSERT_DBG_PARAM(IS_PWR_WAKEUP_PIN(WakeUpPin));
...
void HAL_PWR_EnterSLEEPMode(uint32_t Regulator, uint8_t SleepEntry)
{
  ...
  /* Check the parameter */
  ASSERT_DBG_PARAM(IS_PWR_SLEEP_ENTRY(SleepEntry));
...
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_PWR_ITEMS_ATTRIBUTES(Item));
  ASSERT_DBG_PARAM(IS_PWR_ATTRIBUTES(Attributes));
...
/**
  ...
  * @retval HAL Status.
  */
hal_status_t HAL_PWR_GetConfigAttributes(uint32_t Item, uint32_t *pAttributes)
{
  uint32_t attributes;
