...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPIEx_FlushRxFifo(SPI_HandleTypeDef *hspi)
{
  uint8_t  count  = 0;
...
/**
  ...
  * @retval None
  */
hal_status_t HAL_SPIEx_EnableLockConfiguration(SPI_HandleTypeDef *hspi)
{
...
/**
  ...
  * @retval None
  */
hal_status_t HAL_SPIEx_ConfigureUnderrun(SPI_HandleTypeDef *hspi, uint32_t UnderrunDetection,
                                         uint32_t UnderrunBehaviour)
{
  ...
  ASSERT_DBG_PARAM(IS_SPI_UNDERRUN_BEHAVIOUR(UnderrunBehaviour));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPIEx_SetConfigAutonomousMode(SPI_HandleTypeDef *hspi, SPI_AutonomousModeConfTypeDef *sConfig)
{
  if (hspi->State == HAL_SPI_STATE_READY)
  {
    /* Process Locked */
    ...
    /* Check the parameters */
    ASSERT_DBG_PARAM(IS_SPI_TRIG_SOURCE(hspi->Instance, sConfig->TriggerSelection));

    ASSERT_DBG_PARAM(IS_SPI_AUTO_MODE_TRG_POL(sConfig->TriggerPolarity));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPIEx_GetConfigAutonomousMode(SPI_HandleTypeDef *hspi, SPI_AutonomousModeConfTypeDef *sConfig)
{
  uint32_t autocr_tmp;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPIEx_ClearConfigAutonomousMode(SPI_HandleTypeDef *hspi)
{
  if (hspi->State == HAL_SPI_STATE_READY)
  {
    /* Process Locked */
