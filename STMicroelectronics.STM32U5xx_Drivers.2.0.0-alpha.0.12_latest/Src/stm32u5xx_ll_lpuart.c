...
ErrorStatus LL_LPUART_DeInit(const USART_TypeDef *LPUARTx)
{
  ...
  ASSERT_DBG_PARAM(IS_LPUART_INSTANCE(LPUARTx));
...
ErrorStatus LL_LPUART_Init(USART_TypeDef *LPUARTx, const LL_LPUART_InitTypeDef *LPUART_InitStruct)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_LPUART_INSTANCE(LPUARTx));
  ASSERT_DBG_PARAM(IS_LL_LPUART_PRESCALER(LPUART_InitStruct->PrescalerValue));
  ASSERT_DBG_PARAM(IS_LL_LPUART_BAUDRATE(LPUART_InitStruct->BaudRate));
  ASSERT_DBG_PARAM(IS_LL_LPUART_DATAWIDTH(LPUART_InitStruct->DataWidth));
  ASSERT_DBG_PARAM(IS_LL_LPUART_STOPBITS(LPUART_InitStruct->StopBits));
  ASSERT_DBG_PARAM(IS_LL_LPUART_PARITY(LPUART_InitStruct->Parity));
  ASSERT_DBG_PARAM(IS_LL_LPUART_DIRECTION(LPUART_InitStruct->TransferDirection));
  ASSERT_DBG_PARAM(IS_LL_LPUART_HWCONTROL(LPUART_InitStruct->HardwareFlowControl));
  ...
  if (LL_LPUART_IsEnabled(LPUARTx) == 0U)
  {
    ...
    */
        ...
        && (LPUART_InitStruct->BaudRate != 0U))
    {
      ...
      ASSERT_DBG_PARAM(IS_LL_LPUART_BRR_MIN(LPUARTx->BRR));

      /* Check BRR is lower than or equal to 0xFFFFF */
      ASSERT_DBG_PARAM(IS_LL_LPUART_BRR_MAX(LPUARTx->BRR));
    }
