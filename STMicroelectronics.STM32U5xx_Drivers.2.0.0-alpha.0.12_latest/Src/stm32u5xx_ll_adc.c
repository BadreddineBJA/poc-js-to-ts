...
ErrorStatus LL_ADC_CommonDeInit(ADC_Common_TypeDef *pADCxyCOMMON)
{
  ...
  ASSERT_DBG_PARAM(IS_ADC_COMMON_INSTANCE(pADCxyCOMMON));
...
ErrorStatus LL_ADC_CommonInit(ADC_Common_TypeDef *pADCxyCOMMON, LL_ADC_CommonInitTypeDef *pADC_CommonInit)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_ADC_COMMON_INSTANCE(pADCxyCOMMON));
  ASSERT_DBG_PARAM(IS_LL_ADC_CLOCK(pADC_CommonInit->CommonClock));
...
ErrorStatus LL_ADC_DeInit(ADC_TypeDef *pADCx)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_ADC_ALL_INSTANCE(pADCx));
...
ErrorStatus LL_ADC_Init(ADC_TypeDef *pADCx, LL_ADC_InitTypeDef *pADC_InitStruct)
{
  ...
  ASSERT_DBG_PARAM(IS_LL_ADC_RESOLUTION(pADC_InitStruct->Resolution));
  ASSERT_DBG_PARAM(IS_LL_ADC_LOW_POWER(pADC_InitStruct->LowPowerMode));
  if (pADCx != ADC4) /* ADC1 or ADC2 */
  {
    ASSERT_DBG_PARAM(IS_LL_ADC_LEFT_BIT_SHIFT(pADC_InitStruct->LeftBitShift));
  }
  else
  {
    ASSERT_DBG_PARAM(IS_LL_ADC_DATA_ALIGN(pADC_InitStruct->DataAlignment));
  }
...
ErrorStatus LL_ADC_REG_Init(ADC_TypeDef *pADCx, LL_ADC_REG_InitTypeDef *pADC_RegInitStruct)
{
  ...
  if (pADCx == ADC1)
  {
    ASSERT_DBG_PARAM(IS_LL_ADC_REG_TRIG_SOURCE(pADC_RegInitStruct->TriggerSource));
    ASSERT_DBG_PARAM(IS_LL_ADC_REG_SEQ_SCAN_LENGTH(pADC_RegInitStruct->SequencerLength));
    ASSERT_DBG_PARAM(IS_LL_ADC_REG_DATA_TRANSFER_MODE(pADC_RegInitStruct->DataTransferMode));
  }
  else
  {
    ASSERT_DBG_PARAM(IS_LL_ADC4_REG_TRIG_SOURCE(pADC_RegInitStruct->TriggerSource));
    ASSERT_DBG_PARAM(IS_LL_ADC4_REG_SEQ_SCAN_LENGTH(pADC_RegInitStruct->SequencerLength));
    ASSERT_DBG_PARAM(IS_LL_ADC_REG_DMA_TRANSFER(pADC_RegInitStruct->DMATransfer));
    ...
    {
      ASSERT_DBG_PARAM(IS_LL_ADC_REG_SEQ_SCAN_DISCONT_MODE(pADC_RegInitStruct->SequencerDiscont));

      /* ADC group regular continuous mode and discontinuous mode                 */
      /* can not be enabled simultenaeously                                       */
      ASSERT_DBG_PARAM((pADC_RegInitStruct->ContinuousMode == LL_ADC_REG_CONV_SINGLE)
  ...
  ASSERT_DBG_PARAM(IS_LL_ADC_REG_CONTINUOUS_MODE(pADC_RegInitStruct->ContinuousMode));
  ASSERT_DBG_PARAM(IS_LL_ADC_REG_OVR_DATA_BEHAVIOR(pADC_RegInitStruct->Overrun));
...
ErrorStatus LL_ADC_INJ_Init(ADC_TypeDef *pADCx, LL_ADC_INJ_InitTypeDef *pADC_InjInitStruct)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_ADC_ALL_INSTANCE(pADCx));
  ASSERT_DBG_PARAM(IS_LL_ADC_INJ_TRIG_SOURCE(pADC_InjInitStruct->TriggerSource));
  ASSERT_DBG_PARAM(IS_LL_ADC_INJ_SEQ_SCAN_LENGTH(pADC_InjInitStruct->SequencerLength));
  if (pADC_InjInitStruct->SequencerLength != LL_ADC_INJ_SEQ_SCAN_DISABLE)
  {
    ASSERT_DBG_PARAM(IS_LL_ADC_INJ_SEQ_SCAN_DISCONT_MODE(pADC_InjInitStruct->SequencerDiscont));
  }
  ASSERT_DBG_PARAM(IS_LL_ADC_INJ_TRIG_AUTO(pADC_InjInitStruct->TrigAuto));
