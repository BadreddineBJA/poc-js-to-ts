...
ErrorStatus LL_SPI_DeInit(SPI_TypeDef *SPIx)
{
  ...
  ASSERT_DBG_PARAM(IS_SPI_ALL_INSTANCE(SPIx));
...
ErrorStatus LL_SPI_Init(SPI_TypeDef *SPIx, LL_SPI_InitTypeDef *SPI_InitStruct)
{
  ...
  ASSERT_DBG_PARAM(IS_LL_SPI_TRANSFER_DIRECTION(SPI_InitStruct->TransferDirection));
  ASSERT_DBG_PARAM(IS_LL_SPI_MODE(SPI_InitStruct->Mode));
  ASSERT_DBG_PARAM(IS_LL_SPI_DATAWIDTH(SPI_InitStruct->DataWidth));
  ASSERT_DBG_PARAM(IS_LL_SPI_POLARITY(SPI_InitStruct->ClockPolarity));
  ASSERT_DBG_PARAM(IS_LL_SPI_PHASE(SPI_InitStruct->ClockPhase));
  ASSERT_DBG_PARAM(IS_LL_SPI_NSS(SPI_InitStruct->NSS));
  ASSERT_DBG_PARAM(IS_LL_SPI_BAUDRATEPRESCALER(SPI_InitStruct->BaudRate));
  ASSERT_DBG_PARAM(IS_LL_SPI_BITORDER(SPI_InitStruct->BitOrder));
  ASSERT_DBG_PARAM(IS_LL_SPI_CRCCALCULATION(SPI_InitStruct->CRCCalculation));
  ...
  if (LL_SPI_IsEnabled(SPIx) == 0x00000000UL)
  {
    ...
    {
      ASSERT_DBG_PARAM(IS_LL_SPI_CRC_POLYNOMIAL(SPI_InitStruct->CRCPoly));
