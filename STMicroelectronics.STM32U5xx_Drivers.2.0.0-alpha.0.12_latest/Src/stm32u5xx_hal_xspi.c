...
static hal_status_t XSPI_WaitFlagStateUntilTimeout(XSPI_HandleTypeDef *hxspi, uint32_t Flag, FlagStatus State,
                                                   uint32_t Tickstart, uint32_t Timeout);
static hal_status_t XSPI_ConfigCmd(XSPI_HandleTypeDef *hxspi, XSPI_RegularCmdTypeDef *const pCmd);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Init(XSPI_HandleTypeDef *hxspi)
{
  ...
  else
  {
    ...
    ASSERT_DBG_PARAM(IS_XSPI_MEMORY_MODE(hxspi->Init.MemoryMode));
    ASSERT_DBG_PARAM(IS_XSPI_MEMORY_TYPE(hxspi->Init.MemoryType));
    ASSERT_DBG_PARAM(IS_XSPI_MEMORY_SIZE(hxspi->Init.MemorySize));
    ASSERT_DBG_PARAM(IS_XSPI_CS_HIGH_TIME_CYCLE(hxspi->Init.ChipSelectHighTimeCycle));
    ASSERT_DBG_PARAM(IS_XSPI_FREE_RUN_CLK(hxspi->Init.FreeRunningClock));
    ASSERT_DBG_PARAM(IS_XSPI_CLOCK_MODE(hxspi->Init.ClockMode));
    ASSERT_DBG_PARAM(IS_XSPI_WRAP_SIZE(hxspi->Init.WrapSize));
    ASSERT_DBG_PARAM(IS_XSPI_CLK_PRESCALER(hxspi->Init.ClockPrescaler));
    ASSERT_DBG_PARAM(IS_XSPI_SAMPLE_SHIFTING(hxspi->Init.SampleShifting));
    ASSERT_DBG_PARAM(IS_XSPI_DHQC(hxspi->Init.DelayHoldQuarterCycle));
    ASSERT_DBG_PARAM(IS_XSPI_CS_BOUND(hxspi->Init.ChipSelectBoundary));
    if (IS_OSPI_ALL_INSTANCE(hxspi->Instance))
    {
      ASSERT_DBG_PARAM(IS_OCTOSPI_FIFO_THRESHOLD_BYTE(hxspi->Init.FifoThresholdByte));
    }
    if (IS_HSPI_ALL_INSTANCE(hxspi->Instance))
    {
      ASSERT_DBG_PARAM(IS_HSPI_FIFO_THRESHOLD_BYTE(hxspi->Init.FifoThresholdByte));
    }
    if (IS_OSPI_ALL_INSTANCE(hxspi->Instance))
    {
      ASSERT_DBG_PARAM(IS_XSPI_DLYB_BYPASS(hxspi->Init.DelayBlockBypass));
    }
    if (IS_OSPI_ALL_INSTANCE(hxspi->Instance))
    {
      ASSERT_DBG_PARAM(IS_XSPI_MAXTRAN(hxspi->Init.MaxTran));
    }
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_DeInit(XSPI_HandleTypeDef *hxspi)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Command(XSPI_HandleTypeDef *hxspi, XSPI_RegularCmdTypeDef *const pCmd, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t state;
  uint32_t tickstart = HAL_GetTick();
  ...
  ASSERT_DBG_PARAM(IS_XSPI_OPERATION_TYPE(pCmd->OperationType));
  if (hxspi->Init.MemoryMode == HAL_XSPI_SINGLE_MEM)
  {
    if (IS_OSPI_ALL_INSTANCE(hxspi->Instance))
    {
      ASSERT_DBG_PARAM(IS_OCTOSPI_IO_SELECT(pCmd->IOSelect));
    }
    else if (IS_HSPI_ALL_INSTANCE(hxspi->Instance))
    {
      ASSERT_DBG_PARAM(IS_HSPI_IO_SELECT(pCmd->IOSelect));
    }
    else
    {
  ...
  ASSERT_DBG_PARAM(IS_XSPI_INSTRUCTION_MODE(pCmd->InstructionMode));
  if (pCmd->InstructionMode != HAL_XSPI_INSTRUCTION_NONE)
  {
    ASSERT_DBG_PARAM(IS_XSPI_INSTRUCTION_WIDTH(pCmd->InstructionWidth));
    ASSERT_DBG_PARAM(IS_XSPI_INSTRUCTION_DTR_MODE(pCmd->InstructionDTRMode));
  }

  ASSERT_DBG_PARAM(IS_XSPI_ADDRESS_MODE(pCmd->AddressMode));
  if (pCmd->AddressMode != HAL_XSPI_ADDRESS_NONE)
  {
    ASSERT_DBG_PARAM(IS_XSPI_ADDRESS_WIDTH(pCmd->AddressWidth));
    ASSERT_DBG_PARAM(IS_XSPI_ADDRESS_DTR_MODE(pCmd->AddressDTRMode));
  }

  ASSERT_DBG_PARAM(IS_XSPI_ALT_BYTES_MODE(pCmd->AlternateBytesMode));
  if (pCmd->AlternateBytesMode != HAL_XSPI_ALT_BYTES_NONE)
  {
    ASSERT_DBG_PARAM(IS_XSPI_ALT_BYTES_WIDTH(pCmd->AlternateBytesWidth));
    ASSERT_DBG_PARAM(IS_XSPI_ALT_BYTES_DTR_MODE(pCmd->AlternateBytesDTRMode));
  }

  if (IS_OSPI_ALL_INSTANCE(hxspi->Instance))
  {
    ASSERT_DBG_PARAM(IS_OCTOSPI_DATA_MODE(pCmd->DataMode));
  }
  else if (IS_HSPI_ALL_INSTANCE(hxspi->Instance))
  {
    ASSERT_DBG_PARAM(IS_HSPI_DATA_MODE(pCmd->DataMode));
  }
  else
  {
    hxspi->ErrorCode |= HAL_XSPI_ERROR_INVALID_PARAM;
  ...
  if (pCmd->DataMode != HAL_XSPI_DATA_NONE)
  {
    ...
    {
      ASSERT_DBG_PARAM(IS_XSPI_DATA_LENGTH(pCmd->DataLength));
    }
    ASSERT_DBG_PARAM(IS_XSPI_DATA_DTR_MODE(pCmd->DataDTRMode));
    ASSERT_DBG_PARAM(IS_XSPI_DUMMY_CYCLES(pCmd->DummyCycles));
  }

  ASSERT_DBG_PARAM(IS_XSPI_DQS_MODE(pCmd->DQSMode));
  ASSERT_DBG_PARAM(IS_XSPI_SIOO_MODE(pCmd->SIOOMode));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Command_IT(XSPI_HandleTypeDef *hxspi, XSPI_RegularCmdTypeDef *const pCmd)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_HyperbusCfg(XSPI_HandleTypeDef *hxspi, XSPI_HyperbusCfgTypeDef *const pCfg,
                                       uint32_t Timeout)
{
  hal_status_t status;
  uint32_t state;
  ...
  ASSERT_DBG_PARAM(IS_XSPI_RW_RECOVERY_TIME_CYCLE(pCfg->RWRecoveryTimeCycle));
  ASSERT_DBG_PARAM(IS_XSPI_ACCESS_TIME_CYCLE(pCfg->AccessTimeCycle));
  ASSERT_DBG_PARAM(IS_XSPI_WRITE_ZERO_LATENCY(pCfg->WriteZeroLatency));
  ASSERT_DBG_PARAM(IS_XSPI_LATENCY_MODE(pCfg->LatencyMode));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_HyperbusCmd(XSPI_HandleTypeDef *hxspi, XSPI_HyperbusCmdTypeDef *const pCmd,
                                       uint32_t Timeout)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  ...
  ASSERT_DBG_PARAM(IS_XSPI_ADDRESS_SPACE(pCmd->AddressSpace));
  ASSERT_DBG_PARAM(IS_XSPI_ADDRESS_WIDTH(pCmd->AddressWidth));
  ASSERT_DBG_PARAM(IS_XSPI_DATA_LENGTH(pCmd->DataLength));
  ASSERT_DBG_PARAM(IS_XSPI_DQS_MODE(pCmd->DQSMode));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Transmit(XSPI_HandleTypeDef *hxspi, uint8_t *const pData, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  __IO uint32_t *data_reg = &hxspi->Instance->DR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Receive(XSPI_HandleTypeDef *hxspi, uint8_t *const pData, uint32_t Timeout)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  __IO uint32_t *data_reg = &hxspi->Instance->DR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Transmit_IT(XSPI_HandleTypeDef *hxspi, uint8_t *const pData)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Receive_IT(XSPI_HandleTypeDef *hxspi, uint8_t *const pData)
{
  hal_status_t status = HAL_OK;
  uint32_t addr_reg = hxspi->Instance->AR;
  uint32_t ir_reg = hxspi->Instance->IR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Transmit_DMA(XSPI_HandleTypeDef *hxspi, uint8_t *const pData)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Receive_DMA(XSPI_HandleTypeDef *hxspi, uint8_t *const pData)
{
  hal_status_t status = HAL_OK;
  uint32_t data_size = hxspi->Instance->DLR + 1U;
  uint32_t addr_reg = hxspi->Instance->AR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_AutoPolling(XSPI_HandleTypeDef *hxspi, XSPI_AutoPollingTypeDef *const pCfg,
                                  uint32_t Timeout)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  ...
  ASSERT_DBG_PARAM(IS_XSPI_MATCH_MODE(pCfg->MatchMode));
  ASSERT_DBG_PARAM(IS_XSPI_AUTOMATIC_STOP(pCfg->AutomaticStop));
  ASSERT_DBG_PARAM(IS_XSPI_INTERVAL(pCfg->IntervalTime));
  ASSERT_DBG_PARAM(IS_XSPI_STATUS_BYTES_SIZE(dlr_reg + 1U));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_AutoPolling_IT(XSPI_HandleTypeDef *hxspi, XSPI_AutoPollingTypeDef *const pCfg)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();
  uint32_t addr_reg = hxspi->Instance->AR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_MemoryMapped(XSPI_HandleTypeDef *hxspi, XSPI_MemoryMappedTypeDef *const pCfg)
{
  hal_status_t status;
  uint32_t tickstart = HAL_GetTick();

  /* Check the parameters of the memory-mapped configuration structure */
  ASSERT_DBG_PARAM(IS_XSPI_TIMEOUT_ACTIVATION(pCfg->TimeOutActivation));
  ...
  if (hxspi->State == HAL_XSPI_STATE_CMD_CFG)
  {
    ...
    if (status == HAL_OK)
    {
      ...
      {
        ASSERT_DBG_PARAM(IS_XSPI_TIMEOUT_PERIOD(pCfg->TimeoutPeriodClock));
...
/**
  ...
  * @retval status
  */
hal_status_t HAL_XSPI_RegisterCallback(XSPI_HandleTypeDef *hxspi, HAL_XSPI_CallbackIDTypeDef CallbackID,
...
/**
  ...
  * @retval status
  */
hal_status_t HAL_XSPI_UnRegisterCallback(XSPI_HandleTypeDef *hxspi, HAL_XSPI_CallbackIDTypeDef CallbackID)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Abort(XSPI_HandleTypeDef *hxspi)
{
  hal_status_t status = HAL_OK;
  uint32_t state;
  uint32_t tickstart = HAL_GetTick();
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_Abort_IT(XSPI_HandleTypeDef *hxspi)
{
  hal_status_t status = HAL_OK;
  uint32_t state;
...
/** @brief  Set XSPI Fifo threshold.
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_SetFifoThreshold(XSPI_HandleTypeDef *hxspi, uint32_t Threshold)
{
  hal_status_t status = HAL_OK;
...
/** @brief Set XSPI timeout.
  ...
  * @retval HAL state
  */
hal_status_t HAL_XSPI_SetTimeout(XSPI_HandleTypeDef *hxspi, uint32_t Timeout)
{
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPIM_Config(XSPI_HandleTypeDef *hxspi, XSPIM_CfgTypeDef *const pCfg, uint32_t Timeout)
{
  hal_status_t status = HAL_OK;
  uint32_t instance;
  uint8_t index;
  ...
  ASSERT_DBG_PARAM(IS_XSPIM_PORT(pCfg->ClkPort));
  ASSERT_DBG_PARAM(IS_XSPIM_DQS_PORT(pCfg->DQSPort));
  ASSERT_DBG_PARAM(IS_XSPIM_PORT(pCfg->NCSPort));
  ASSERT_DBG_PARAM(IS_XSPIM_IO_PORT(pCfg->IOLowPort));
  ASSERT_DBG_PARAM(IS_XSPIM_IO_PORT(pCfg->IOHighPort));
  ASSERT_DBG_PARAM(IS_XSPIM_REQ2ACKTIME(pCfg->Req2AckTime));
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_XSPI_DLYB_SetConfig(XSPI_HandleTypeDef *hxspi, HAL_XSPI_DLYB_CfgTypeDef  *const pdlyb_cfg)
{
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_XSPI_DLYB_GetConfig(XSPI_HandleTypeDef *hxspi, HAL_XSPI_DLYB_CfgTypeDef  *const pdlyb_cfg)
{
  hal_status_t status = HAL_ERROR;
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_XSPI_DLYB_GetClockPeriod(XSPI_HandleTypeDef *hxspi, HAL_XSPI_DLYB_CfgTypeDef  *const pdlyb_cfg)
{
  hal_status_t status = HAL_ERROR;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_GetDelayValue(XSPI_HandleTypeDef *hxspi, XSPI_HSCalTypeDef *const pCfg)
{
  hal_status_t status = HAL_OK;
  __IO uint32_t reg = 0;
  ...
  if (IS_HSPI_ALL_INSTANCE(hxspi->Instance))
  {
    ...
    ASSERT_DBG_PARAM(IS_XSPI_DELAY_TYPE(pCfg->DelayValueType));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_XSPI_SetDelayValue(XSPI_HandleTypeDef *hxspi, XSPI_HSCalTypeDef *const pCfg)
{
  hal_status_t status = HAL_OK;
  ...
  if (IS_HSPI_ALL_INSTANCE(hxspi->Instance))
  {
    /* Check the parameter specified in the structure */
    ASSERT_DBG_PARAM(IS_XSPI_DELAY_TYPE(pCfg->DelayValueType));
    ASSERT_DBG_PARAM(IS_XSPI_FINECAL_VALUE(pCfg->FineCalibrationUnit));
    ASSERT_DBG_PARAM(IS_XSPI_COARSECAL_VALUE(pCfg->CoarseCalibrationUnit));
...
/**
  ...
  * @retval HAL status
  */
static hal_status_t XSPI_WaitFlagStateUntilTimeout(XSPI_HandleTypeDef *hxspi, uint32_t Flag,
...
/**
  ...
  * @retval HAL status
  */
static hal_status_t XSPI_ConfigCmd(XSPI_HandleTypeDef *hxspi, XSPI_RegularCmdTypeDef *pCmd)
{
  hal_status_t status = HAL_OK;
