...
ErrorStatus LL_USART_DeInit(const USART_TypeDef *USARTx)
{
  ...
  ASSERT_DBG_PARAM(IS_UART_INSTANCE(USARTx));
...
ErrorStatus LL_USART_Init(USART_TypeDef *USARTx, const LL_USART_InitTypeDef *USART_InitStruct)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_UART_INSTANCE(USARTx));
  ASSERT_DBG_PARAM(IS_LL_USART_PRESCALER(USART_InitStruct->PrescalerValue));
  ASSERT_DBG_PARAM(IS_LL_USART_BAUDRATE(USART_InitStruct->BaudRate));
  ASSERT_DBG_PARAM(IS_LL_USART_DATAWIDTH(USART_InitStruct->DataWidth));
  ASSERT_DBG_PARAM(IS_LL_USART_STOPBITS(USART_InitStruct->StopBits));
  ASSERT_DBG_PARAM(IS_LL_USART_PARITY(USART_InitStruct->Parity));
  ASSERT_DBG_PARAM(IS_LL_USART_DIRECTION(USART_InitStruct->TransferDirection));
  ASSERT_DBG_PARAM(IS_LL_USART_HWCONTROL(USART_InitStruct->HardwareFlowControl));
  ASSERT_DBG_PARAM(IS_LL_USART_OVERSAMPLING(USART_InitStruct->OverSampling));
  ...
  if (LL_USART_IsEnabled(USARTx) == 0U)
  {
    ...
    */
        ...
        && (USART_InitStruct->BaudRate != 0U))
    {
      ...
      ASSERT_DBG_PARAM(IS_LL_USART_BRR_MIN(USARTx->BRR));
    }
...
ErrorStatus LL_USART_ClockInit(USART_TypeDef *USARTx, const LL_USART_ClockInitTypeDef *USART_ClockInitStruct)
{
  ...
  ASSERT_DBG_PARAM(IS_UART_INSTANCE(USARTx));
  ASSERT_DBG_PARAM(IS_LL_USART_CLOCKOUTPUT(USART_ClockInitStruct->ClockOutput));
  ...
  if (LL_USART_IsEnabled(USARTx) == 0U)
  {
    ...
    ASSERT_DBG_PARAM(IS_USART_INSTANCE(USARTx));

    /* Check clock related parameters */
    ASSERT_DBG_PARAM(IS_LL_USART_CLOCKPOLARITY(USART_ClockInitStruct->ClockPolarity));
    ASSERT_DBG_PARAM(IS_LL_USART_CLOCKPHASE(USART_ClockInitStruct->ClockPhase));
    ASSERT_DBG_PARAM(IS_LL_USART_LASTBITCLKOUTPUT(USART_ClockInitStruct->LastBitClockPulse));
