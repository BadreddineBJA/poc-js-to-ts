...
ErrorStatus LL_DAC_DeInit(DAC_TypeDef *DACx)
{
  ...
  ASSERT_DBG_PARAM(IS_DAC_ALL_INSTANCE(DACx));
...
ErrorStatus LL_DAC_Init(DAC_TypeDef *DACx, uint32_t DAC_Channel, LL_DAC_InitTypeDef *DAC_InitStruct)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_DAC_ALL_INSTANCE(DACx));
  ASSERT_DBG_PARAM(IS_LL_DAC_CHANNEL(DAC_Channel));
  ASSERT_DBG_PARAM(IS_LL_DAC_TRIGGER_SOURCE(DAC_InitStruct->TriggerSource));
  ASSERT_DBG_PARAM(IS_LL_DAC_OUTPUT_BUFFER(DAC_InitStruct->OutputBuffer));
  ASSERT_DBG_PARAM(IS_LL_DAC_OUTPUT_CONNECTION(DAC_InitStruct->OutputConnection));
  ASSERT_DBG_PARAM(IS_LL_DAC_OUTPUT_MODE(DAC_InitStruct->OutputMode));
  ASSERT_DBG_PARAM(IS_LL_DAC_WAVE_AUTO_GENER_MODE(DAC_InitStruct->WaveAutoGeneration));
  if (DAC_InitStruct->WaveAutoGeneration != LL_DAC_WAVE_AUTO_GENERATION_NONE)
  {
    ASSERT_DBG_PARAM(IS_LL_DAC_WAVE_AUTO_GENER_CONFIG(DAC_InitStruct->WaveAutoGeneration,
