...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_Init(DMA_HandleTypeDef *const hdma)
{
  /* Get tick number */
  ...
  ASSERT_DBG_PARAM(IS_DMA_DIRECTION(hdma->Init.Direction));
  if ((hdma->Init.Direction == DMA_MEMORY_TO_PERIPH) || (hdma->Init.Direction == DMA_MEMORY_TO_MEMORY))
  {
    ASSERT_DBG_PARAM(IS_DMA_REQUEST(hdma->Init.Request));
  }
  ASSERT_DBG_PARAM(IS_DMA_BLOCK_HW_REQUEST(hdma->Init.BlkHWRequest));
  ASSERT_DBG_PARAM(IS_DMA_SOURCE_INC(hdma->Init.SrcInc));
  ASSERT_DBG_PARAM(IS_DMA_DESTINATION_INC(hdma->Init.DestInc));
  ASSERT_DBG_PARAM(IS_DMA_SOURCE_DATA_WIDTH(hdma->Init.SrcDataWidth));
  ASSERT_DBG_PARAM(IS_DMA_DESTINATION_DATA_WIDTH(hdma->Init.DestDataWidth));
  ASSERT_DBG_PARAM(IS_DMA_PRIORITY(hdma->Init.Priority));
  ASSERT_DBG_PARAM(IS_DMA_TCEM_EVENT_MODE(hdma->Init.TransferEventMode));
  ASSERT_DBG_PARAM(IS_DMA_MODE(hdma->Init.Mode));
  /* Check DMA channel instance */
  if (IS_GPDMA_INSTANCE(hdma->Instance) != 0U)
  {
    ASSERT_DBG_PARAM(IS_DMA_BURST_LENGTH(hdma->Init.SrcBurstLength));
    ASSERT_DBG_PARAM(IS_DMA_BURST_LENGTH(hdma->Init.DestBurstLength));
    ASSERT_DBG_PARAM(IS_DMA_TRANSFER_ALLOCATED_PORT(hdma->Init.TransferAllocatedPort));
  }
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_DeInit(DMA_HandleTypeDef *const hdma)
{
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_Start(DMA_HandleTypeDef *const hdma,
                                uint32_t SrcAddress,
                                uint32_t DstAddress,
                                uint32_t SrcDataSize)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_DMA_BLOCK_SIZE(SrcDataSize));
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_Start_IT(DMA_HandleTypeDef *const hdma,
                                   uint32_t SrcAddress,
                                   uint32_t DstAddress,
                                   uint32_t SrcDataSize)
{
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_Abort(DMA_HandleTypeDef *const hdma)
{
  /* Get tick number */
  uint32_t tickstart =  HAL_GetTick();
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_Abort_IT(DMA_HandleTypeDef *const hdma)
{
  /* Check the DMA peripheral handle parameter */
  if (hdma == NULL)
  {
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DMA_PollForTransfer(DMA_HandleTypeDef *const hdma,
                                          ...
                                          uint32_t Timeout)
{
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_DMA_LEVEL_COMPLETE(CompleteLevel));
...
{
  dma_t *p_dma_instance = GET_DMA_INSTANCE(hdma);
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_RegisterCallback(DMA_HandleTypeDef *const hdma,
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_UnRegisterCallback(DMA_HandleTypeDef *const hdma,
                                             HAL_DMA_CallbackIDTypeDef CallbackID)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL Status.
  */
hal_status_t HAL_DMA_ConfigChannelAttributes(DMA_HandleTypeDef *const hdma, uint32_t ChannelAttributes)
{
  dma_t *p_dma_instance;
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_DMA_ATTRIBUTES(ChannelAttributes));
...
/**
  ...
  * @retval HAL Status.
  */
hal_status_t HAL_DMA_GetConfigChannelAttributes(DMA_HandleTypeDef const *const hdma,
...
/**
  ...
  * @retval HAL Status.
  */
hal_status_t HAL_DMA_LockChannelAttributes(DMA_HandleTypeDef const *const hdma)
{
  dma_t *p_dma_instance;
  uint32_t channel_idx;
...
/**
  ...
  * @retval HAL status.
  */
hal_status_t HAL_DMA_GetLockChannelAttributes(DMA_HandleTypeDef const *const hdma, uint32_t *const pLockState)
{
  dma_t *p_dma_instance;
  uint32_t channel_idx;
