...
/**
  ...
  * @retval None
  */
void HAL_GPIO_Init(gpio_t  *GPIOx, const GPIO_InitTypeDef *pGPIO_Init)
{
  uint32_t tmp;
  uint32_t iocurrent;
  ...
  ASSERT_DBG_PARAM(IS_GPIO_ALL_INSTANCE(GPIOx));
  ASSERT_DBG_PARAM(IS_GPIO_PIN(pGPIO_Init->Pin));
  ASSERT_DBG_PARAM(IS_GPIO_MODE(pGPIO_Init->Mode));
  ...
  while (((pGPIO_Init->Pin) >> position) != 0U)
  {
    ...
    if (iocurrent != 0U)
    {
      ...
      else if ((pGPIO_Init->Mode == GPIO_MODE_AF_PP) || (pGPIO_Init->Mode == GPIO_MODE_AF_OD))
      {
        /* Check the parameters */
        ASSERT_DBG_PARAM(IS_GPIO_ALL_INSTANCE(GPIOx));
        ASSERT_DBG_PARAM(IS_GPIO_AF_INSTANCE(GPIOx));
        ASSERT_DBG_PARAM(IS_GPIO_AF(pGPIO_Init->Alternate));
          ...
          (pGPIO_Init->Mode == GPIO_MODE_OUTPUT_OD) || (pGPIO_Init->Mode == GPIO_MODE_AF_OD))
      {
        ...
        ASSERT_DBG_PARAM(IS_GPIO_SPEED(pGPIO_Init->Speed));
      ...
      if (pGPIO_Init->Mode != GPIO_MODE_ANALOG)
      {
        ...
        ASSERT_DBG_PARAM(IS_GPIO_PULL(pGPIO_Init->Pull));
...
/**
  ...
  * @retval None
  */
void HAL_GPIO_DeInit(gpio_t  *GPIOx, uint32_t GPIO_Pin)
{
  uint32_t tmp;
  uint32_t iocurrent;
  uint32_t pin_position;
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_GPIO_PIN(GPIO_Pin));
...
/**
  ...
  */
GPIO_PinState HAL_GPIO_ReadPin(gpio_t *GPIOx, uint16_t GPIO_Pin)
{
...
/**
  ...
  * @retval None
  */
void HAL_GPIO_WritePin(gpio_t *GPIOx, uint16_t GPIO_Pin, GPIO_PinState PinState)
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_GPIO_PIN(GPIO_Pin));
  ASSERT_DBG_PARAM(IS_GPIO_PIN_ACTION(PinState));
...
/**
  ...
  * @retval None
  */
void HAL_GPIO_TogglePin(gpio_t *GPIOx, uint16_t GPIO_Pin)
{
  uint32_t odr;
...
/**
  ...
  * @retval None
  */
hal_status_t HAL_GPIO_LockPin(gpio_t *GPIOx, uint16_t GPIO_Pin)
{
  uint32_t iocurrent;
  uint32_t pin_locked;
  uint32_t pin_position;
...
/**
  ...
  * @retval None
  */
void HAL_GPIO_EnableHighSPeedLowVoltage(gpio_t *GPIOx, uint16_t GPIO_Pin)
{
  uint32_t iocurrent;
  uint32_t pin_position;
  uint32_t position = 0U;
...
/**
  ...
  * @retval None
  */
void HAL_GPIO_DisableHighSPeedLowVoltage(gpio_t *GPIOx, uint16_t GPIO_Pin)
{
  uint32_t iocurrent;
  uint32_t pin_position;
  uint32_t position = 0U;
...
/**
  ...
  * @retval None
  */
void HAL_GPIO_ConfigPinAttributes(gpio_t *GPIOx, uint16_t GPIO_Pin, uint32_t PinAttributes)
{
  uint32_t iocurrent;
  uint32_t pin_position;
  uint32_t position = 0U;
  ...
  ASSERT_DBG_PARAM(IS_GPIO_ALL_INSTANCE(GPIOx));
  ASSERT_DBG_PARAM(IS_GPIO_PIN(GPIO_Pin));
  ASSERT_DBG_PARAM(IS_GPIO_PIN_ATTRIBUTES(PinAttributes));
...
/**
  ...
  * @retval HAL Status.
  */
hal_status_t HAL_GPIO_GetConfigPinAttributes(gpio_t *GPIOx, uint16_t GPIO_Pin, uint32_t *pPinAttributes)
{
  uint32_t iocurrent;
  uint32_t pin_position;
  uint32_t position = 0U;
  ...
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_GPIO_ALL_INSTANCE(GPIOx));
  ASSERT_DBG_PARAM(IS_GPIO_PIN(GPIO_Pin) && (GPIO_Pin != GPIO_PIN_ALL));
