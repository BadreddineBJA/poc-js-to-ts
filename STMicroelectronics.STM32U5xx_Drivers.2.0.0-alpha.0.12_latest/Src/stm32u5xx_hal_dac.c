...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DAC_Init(DAC_HandleTypeDef *hdac)
{
  /* Check DAC handle */
  if (hdac == NULL)
  {
  ...
  ASSERT_DBG_PARAM(IS_DAC_ALL_INSTANCE(hdac->Instance));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DAC_DeInit(DAC_HandleTypeDef *hdac)
{
  /* Check DAC handle */
  if (hdac == NULL)
  {
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DAC_Start(DAC_HandleTypeDef *hdac, uint32_t Channel)
{
  __IO uint32_t wait_loop_index;
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_DAC_CHANNEL(Channel));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DAC_Stop(DAC_HandleTypeDef *hdac, uint32_t Channel)
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_DAC_CHANNEL(Channel));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DAC_Start_DMA(DAC_HandleTypeDef *hdac, uint32_t Channel, uint32_t *pData, uint32_t Length,
                                    uint32_t Alignment)
{
  hal_status_t status;
  uint32_t tmpreg;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DAC_Stop_DMA(DAC_HandleTypeDef *hdac, uint32_t Channel)
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_DAC_CHANNEL(Channel));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DAC_SetValue(DAC_HandleTypeDef *hdac, uint32_t Channel, uint32_t Alignment, uint32_t Data)
{
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_DAC_ConfigChannel(DAC_HandleTypeDef *hdac, DAC_ChannelConfTypeDef *sConfig, uint32_t Channel)
{
  uint32_t tmpreg1;
  uint32_t tmpreg2;
  uint32_t tickstart;
  ...
  ASSERT_DBG_PARAM(IS_DAC_AUTONOMOUS(sConfig->DAC_AutonomousMode));
  ASSERT_DBG_PARAM(IS_DAC_HIGH_FREQUENCY_MODE(sConfig->DAC_HighFrequency));
  ASSERT_DBG_PARAM(IS_DAC_TRIGGER(sConfig->DAC_Trigger));
  ASSERT_DBG_PARAM(IS_DAC_OUTPUT_BUFFER_STATE(sConfig->DAC_OutputBuffer));
  ASSERT_DBG_PARAM(IS_DAC_CHIP_CONNECTION(sConfig->DAC_ConnectOnChipPeripheral));
  ASSERT_DBG_PARAM(IS_DAC_TRIMMING(sConfig->DAC_UserTrimming));
  if ((sConfig->DAC_UserTrimming) == DAC_TRIMMING_USER)
  {
    ASSERT_DBG_PARAM(IS_DAC_TRIMMINGVALUE(sConfig->DAC_TrimmingValue));
  }
  ASSERT_DBG_PARAM(IS_DAC_SAMPLEANDHOLD(sConfig->DAC_SampleAndHold));
  if ((sConfig->DAC_SampleAndHold) == DAC_SAMPLEANDHOLD_ENABLE)
  {
    ASSERT_DBG_PARAM(IS_DAC_SAMPLETIME(sConfig->DAC_SampleAndHoldConfig.DAC_SampleTime));
    ASSERT_DBG_PARAM(IS_DAC_HOLDTIME(sConfig->DAC_SampleAndHoldConfig.DAC_HoldTime));
    ASSERT_DBG_PARAM(IS_DAC_REFRESHTIME(sConfig->DAC_SampleAndHoldConfig.DAC_RefreshTime));
  }
  ASSERT_DBG_PARAM(IS_DAC_CHANNEL(Channel));
  ASSERT_DBG_PARAM(IS_FUNCTIONAL_STATE(sConfig->DAC_DMADoubleDataMode));
  ASSERT_DBG_PARAM(IS_FUNCTIONAL_STATE(sConfig->DAC_SignedFormat));
...
/**
  ...
  * @retval status
  */
hal_status_t HAL_DAC_RegisterCallback(DAC_HandleTypeDef *hdac, HAL_DAC_CallbackIDTypeDef CallbackID,
...
/**
  ...
  * @retval status
  */
hal_status_t HAL_DAC_UnRegisterCallback(DAC_HandleTypeDef *hdac, HAL_DAC_CallbackIDTypeDef CallbackID)
{
  hal_status_t status = HAL_OK;
