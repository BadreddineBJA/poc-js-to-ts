...
static hal_status_t SMBUS_WaitOnFlagUntilTimeout(SMBUS_HandleTypeDef *hsmbus, uint32_t Flag,
                                                 FlagStatus Status, uint32_t Timeout);

/* Private functions for SMBUS transfer IRQ handler */
static hal_status_t SMBUS_Master_ISR(SMBUS_HandleTypeDef *hsmbus, uint32_t StatusFlags);
static hal_status_t SMBUS_Slave_ISR(SMBUS_HandleTypeDef *hsmbus, uint32_t StatusFlags);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_Init(SMBUS_HandleTypeDef *hsmbus)
{
  ...
  ASSERT_DBG_PARAM(IS_SMBUS_ALL_INSTANCE(hsmbus->Instance));
  ASSERT_DBG_PARAM(IS_SMBUS_ANALOG_FILTER(hsmbus->Init.AnalogFilter));
  ASSERT_DBG_PARAM(IS_SMBUS_OWN_ADDRESS1(hsmbus->Init.OwnAddress1));
  ASSERT_DBG_PARAM(IS_SMBUS_ADDRESSING_MODE(hsmbus->Init.AddressingMode));
  ASSERT_DBG_PARAM(IS_SMBUS_DUAL_ADDRESS(hsmbus->Init.DualAddressMode));
  ASSERT_DBG_PARAM(IS_SMBUS_OWN_ADDRESS2(hsmbus->Init.OwnAddress2));
  ASSERT_DBG_PARAM(IS_SMBUS_OWN_ADDRESS2_MASK(hsmbus->Init.OwnAddress2Masks));
  ASSERT_DBG_PARAM(IS_SMBUS_GENERAL_CALL(hsmbus->Init.GeneralCallMode));
  ASSERT_DBG_PARAM(IS_SMBUS_NO_STRETCH(hsmbus->Init.NoStretchMode));
  ASSERT_DBG_PARAM(IS_SMBUS_PEC(hsmbus->Init.PacketErrorCheckMode));
  ASSERT_DBG_PARAM(IS_SMBUS_PERIPHERAL_MODE(hsmbus->Init.PeripheralMode));
  ...
  {
    if (hsmbus->Init.AddressingMode == HAL_SMBUS_ADDRESSINGMODE_7BIT)
    {
  ...
  if ((hsmbus->Init.PacketErrorCheckMode == HAL_SMBUS_PEC_ENABLE) && \
      ((hsmbus->Init.PeripheralMode == HAL_SMBUS_PERIPHERAL_MODE_SMBUS_SLAVE) || \
       (hsmbus->Init.PeripheralMode == HAL_SMBUS_PERIPHERAL_MODE_SMBUS_SLAVE_ARP)))
  {
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_DeInit(SMBUS_HandleTypeDef *hsmbus)
{
  /* Check the SMBUS handle allocation */
  if (hsmbus == NULL)
  {
...
/**
  ...
  *         @arg @ref HAL_SMBUS_ANALOGFILTER_ENABLE
  *         @arg @ref HAL_SMBUS_ANALOGFILTER_DISABLE
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_ConfigAnalogFilter(SMBUS_HandleTypeDef *hsmbus, uint32_t AnalogFilter)
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SMBUS_ALL_INSTANCE(hsmbus->Instance));
  ASSERT_DBG_PARAM(IS_SMBUS_ANALOG_FILTER(AnalogFilter));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_ConfigDigitalFilter(SMBUS_HandleTypeDef *hsmbus, uint32_t DigitalFilter)
{
  uint32_t tmpreg;

  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SMBUS_ALL_INSTANCE(hsmbus->Instance));
  ASSERT_DBG_PARAM(IS_SMBUS_DIGITAL_FILTER(DigitalFilter));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_RegisterCallback(SMBUS_HandleTypeDef *hsmbus,
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_UnRegisterCallback(SMBUS_HandleTypeDef *hsmbus,
                                          HAL_SMBUS_CallbackIDTypeDef CallbackID)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_RegisterAddrCallback(SMBUS_HandleTypeDef *hsmbus,
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_UnRegisterAddrCallback(SMBUS_HandleTypeDef *hsmbus)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_Master_Transmit_IT(SMBUS_HandleTypeDef *hsmbus, uint16_t DevAddress,
                                          uint8_t *pData, uint16_t Size, uint32_t XferOptions)
{
  uint32_t tmp;

  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SMBUS_TRANSFER_OPTIONS_REQUEST(XferOptions));
  ...
  if (hsmbus->State == HAL_SMBUS_STATE_READY)
  {
    ...
    {
      hsmbus->XferOptions &= ~HAL_SMBUS_AUTOEND_MODE;
    }
    ...
    if ((hsmbus->XferSize < hsmbus->XferCount) && (hsmbus->XferSize == MAX_NBYTE_SIZE))
    {
      ...
      if ((hsmbus->Instance->AUTOCR & I2C_AUTOCR_TRIGEN) == I2C_AUTOCR_TRIGEN)
      {
        SMBUS_TransferConfig(hsmbus, DevAddress, (uint8_t)hsmbus->XferSize,
                             HAL_SMBUS_RELOAD_MODE | (hsmbus->XferOptions & HAL_SMBUS_SENDPEC_MODE),
                             HAL_SMBUS_GENERATE_NO_START_WRITE);
      }
      else
      {
        SMBUS_TransferConfig(hsmbus, DevAddress, (uint8_t)hsmbus->XferSize,
                             HAL_SMBUS_RELOAD_MODE | (hsmbus->XferOptions & HAL_SMBUS_SENDPEC_MODE),
                             HAL_SMBUS_GENERATE_START_WRITE);
      }
    }
    else
    {
      ...
      if (HAL_SMBUS_GET_PEC_MODE(hsmbus) != 0UL)
      {
        hsmbus->XferSize--;
        hsmbus->XferCount--;
      }
    ...
    /* Note : The SMBUS interrupts must be enabled after unlocking current process
              ...
              process unlock */
    SMBUS_Enable_IRQ(hsmbus, HAL_SMBUS_IT_TX);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_Master_Receive_IT(SMBUS_HandleTypeDef *hsmbus, uint16_t DevAddress, uint8_t *pData,
  ...
  if (hsmbus->State == HAL_SMBUS_STATE_READY)
  {
    ...
    if ((hsmbus->XferSize < hsmbus->XferCount) && (hsmbus->XferSize == MAX_NBYTE_SIZE))
    {
      ...
      if ((hsmbus->Instance->AUTOCR & I2C_AUTOCR_TRIGEN) == I2C_AUTOCR_TRIGEN)
      {
        SMBUS_TransferConfig(hsmbus, DevAddress, (uint8_t)hsmbus->XferSize,
                             HAL_SMBUS_RELOAD_MODE  | (hsmbus->XferOptions & HAL_SMBUS_SENDPEC_MODE),
                             HAL_SMBUS_GENERATE_NO_START_READ);
      }
      else
      {
        SMBUS_TransferConfig(hsmbus, DevAddress, (uint8_t)hsmbus->XferSize,
                             HAL_SMBUS_RELOAD_MODE  | (hsmbus->XferOptions & HAL_SMBUS_SENDPEC_MODE),
                             HAL_SMBUS_GENERATE_START_READ);
      }
    }
    else
    {
    ...
    /* Note : The SMBUS interrupts must be enabled after unlocking current process
              to avoid the risk of SMBUS interrupt handle execution before current
              process unlock */
    SMBUS_Enable_IRQ(hsmbus, HAL_SMBUS_IT_RX);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_Master_Abort_IT(SMBUS_HandleTypeDef *hsmbus, uint16_t DevAddress)
{
  if (hsmbus->State == HAL_SMBUS_STATE_READY)
  {
    /* Process Locked */
    ...
    SMBUS_TransferConfig(hsmbus, DevAddress, 1, HAL_SMBUS_AUTOEND_MODE, HAL_SMBUS_NO_STARTSTOP);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_Slave_Transmit_IT(SMBUS_HandleTypeDef *hsmbus, uint8_t *pData, uint16_t Size,
                                         uint32_t XferOptions)
{
  ...
  if ((hsmbus->State & HAL_SMBUS_STATE_LISTEN) == HAL_SMBUS_STATE_LISTEN)
  {
    ...
    SMBUS_Disable_IRQ(hsmbus, HAL_SMBUS_IT_ADDR | HAL_SMBUS_IT_TX);
    ...
    __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_ADDR);
    ...
    SMBUS_Enable_IRQ(hsmbus, HAL_SMBUS_IT_TX | HAL_SMBUS_IT_ADDR);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_Slave_Receive_IT(SMBUS_HandleTypeDef *hsmbus, uint8_t *pData, uint16_t Size,
                                        uint32_t XferOptions)
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SMBUS_TRANSFER_OPTIONS_REQUEST(XferOptions));
  ...
  if ((hsmbus->State & HAL_SMBUS_STATE_LISTEN) == HAL_SMBUS_STATE_LISTEN)
  {
    ...
    /* Disable Interrupts, to prevent preemption during treatment in case of multicall */
    SMBUS_Disable_IRQ(hsmbus, HAL_SMBUS_IT_ADDR | HAL_SMBUS_IT_RX);
    ...
    if (((HAL_SMBUS_GET_PEC_MODE(hsmbus) != 0UL) && (hsmbus->XferSize == 2U)) || (hsmbus->XferSize == 1U))
    {
      SMBUS_TransferConfig(hsmbus, 0, (uint8_t)hsmbus->XferSize, hsmbus->XferOptions,
                           HAL_SMBUS_NO_STARTSTOP);
    }
    ...
    {
      SMBUS_TransferConfig(hsmbus, 0, 1, hsmbus->XferOptions | HAL_SMBUS_RELOAD_MODE, HAL_SMBUS_NO_STARTSTOP);
    }
    ...
    /* Note : The SMBUS interrupts must be enabled after unlocking current process
              ...
              process unlock */
    /* REnable ADDR interrupt */
    SMBUS_Enable_IRQ(hsmbus, HAL_SMBUS_IT_RX | HAL_SMBUS_IT_ADDR);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_EnableListen_IT(SMBUS_HandleTypeDef *hsmbus)
{
  hsmbus->State = HAL_SMBUS_STATE_LISTEN;

  /* Enable the Address Match interrupt */
  SMBUS_Enable_IRQ(hsmbus, HAL_SMBUS_IT_ADDR);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_DisableListen_IT(SMBUS_HandleTypeDef *hsmbus)
{
  ...
  if (hsmbus->State == HAL_SMBUS_STATE_LISTEN)
  {
    ...
    SMBUS_Disable_IRQ(hsmbus, HAL_SMBUS_IT_ADDR);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_EnableAlert_IT(SMBUS_HandleTypeDef *hsmbus)
{
  /* Enable SMBus alert */
  hsmbus->Instance->CR1 |= I2C_CR1_ALERTEN;

  /* Clear ALERT flag */
  __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_ALERT);

  /* Enable Alert Interrupt */
  SMBUS_Enable_IRQ(hsmbus, HAL_SMBUS_IT_ALERT);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_DisableAlert_IT(SMBUS_HandleTypeDef *hsmbus)
{
  /* Enable SMBus alert */
  hsmbus->Instance->CR1 &= ~I2C_CR1_ALERTEN;

  /* Disable Alert Interrupt */
  SMBUS_Disable_IRQ(hsmbus, HAL_SMBUS_IT_ALERT);
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SMBUS_IsDeviceReady(SMBUS_HandleTypeDef *hsmbus, uint16_t DevAddress, uint32_t Trials,
                                     uint32_t Timeout)
{
  uint32_t tickstart;
  ...
  if (hsmbus->State == HAL_SMBUS_STATE_READY)
  {
    if (__HAL_SMBUS_GET_FLAG(hsmbus, HAL_SMBUS_FLAG_BUSY) != RESET)
    {
      return HAL_BUSY;
    }
    ...
    {
      /* Generate Start */
      hsmbus->Instance->CR2 = HAL_SMBUS_GENERATE_START(hsmbus->Init.AddressingMode, DevAddress);

      /* No need to Check TC flag, with AUTOEND mode the stop is automatically generated */
      /* Wait until STOPF flag is set or a NACK flag is set*/
      tickstart = HAL_GetTick();

      tmp1 = __HAL_SMBUS_GET_FLAG(hsmbus, HAL_SMBUS_FLAG_STOPF);
      tmp2 = __HAL_SMBUS_GET_FLAG(hsmbus, HAL_SMBUS_FLAG_AF);
      ...
      if (__HAL_SMBUS_GET_FLAG(hsmbus, HAL_SMBUS_FLAG_AF) == RESET)
      {
        /* Wait until STOPF flag is reset */
        if (SMBUS_WaitOnFlagUntilTimeout(hsmbus, HAL_SMBUS_FLAG_STOPF, RESET, Timeout) != HAL_OK)
        {
          return HAL_ERROR;
        }

        /* Clear STOP Flag */
        __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_STOPF);
      ...
      else
      {
        ...
        /* Clear NACK Flag */
        __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_AF);
...
void HAL_SMBUS_EV_IRQHandler(SMBUS_HandleTypeDef *hsmbus)
{
  ...
  if ((HAL_SMBUS_CHECK_IT_SOURCE(tmpcr1value, (HAL_SMBUS_IT_TCI | HAL_SMBUS_IT_STOPI |
                                               HAL_SMBUS_IT_NACKI | HAL_SMBUS_IT_TXI)) != RESET) &&
      ((HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_TXIS) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_TCR) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_TC) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_STOPF) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_AF) != RESET)))
  {
    /* Slave mode selected */
  ...
  if ((HAL_SMBUS_CHECK_IT_SOURCE(tmpcr1value, (HAL_SMBUS_IT_TCI | HAL_SMBUS_IT_STOPI |
                                               HAL_SMBUS_IT_NACKI | HAL_SMBUS_IT_RXI)) != RESET) &&
      ((HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_RXNE) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_TCR) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_TC) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_STOPF) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_AF) != RESET)))
  ...
  if (((HAL_SMBUS_CHECK_IT_SOURCE(tmpcr1value, HAL_SMBUS_IT_ADDRI) != RESET) ||
       (HAL_SMBUS_CHECK_IT_SOURCE(tmpcr1value, HAL_SMBUS_IT_STOPI) != RESET) ||
       (HAL_SMBUS_CHECK_IT_SOURCE(tmpcr1value, HAL_SMBUS_IT_NACKI) != RESET)) &&
      ((HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_ADDR) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_STOPF) != RESET) ||
       (HAL_SMBUS_CHECK_FLAG(tmpisrvalue, HAL_SMBUS_FLAG_AF) != RESET)))
  {
    if ((hsmbus->State & HAL_SMBUS_STATE_LISTEN) == HAL_SMBUS_STATE_LISTEN)
...
static hal_status_t SMBUS_Master_ISR(SMBUS_HandleTypeDef *hsmbus, uint32_t StatusFlags)
{
  ...
  if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_AF) != RESET)
  {
    /* Clear NACK Flag */
    __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_AF);
  ...
  else if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_STOPF) != RESET)
  {
    ...
    {
      /* Disable Interrupt */
      SMBUS_Disable_IRQ(hsmbus, HAL_SMBUS_IT_TX);

      /* Clear STOP Flag */
      __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_STOPF);

      /* Clear Configuration Register 2 */
      HAL_SMBUS_RESET_CR2(hsmbus);
    ...
    else if (hsmbus->State == HAL_SMBUS_STATE_MASTER_BUSY_RX)
    {
      ...
      if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_RXNE) != RESET)
      {
        /* Read data from RXDR */
      ...
      /* Disable Interrupt */
      SMBUS_Disable_IRQ(hsmbus, HAL_SMBUS_IT_RX);
  ...
  else if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_RXNE) != RESET)
  {
    /* Read data from RXDR */
    *hsmbus->pBuffPtr = (uint8_t)(hsmbus->Instance->RXDR);
  ...
  else if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_TXIS) != RESET)
  {
    /* Write data to TXDR */
  ...
  else if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_TCR) != RESET)
  {
    ...
    else if ((hsmbus->XferCount == 0U) && (hsmbus->XferSize == 0U))
    {
      ...
      if (HAL_SMBUS_GET_STOP_MODE(hsmbus) != HAL_SMBUS_AUTOEND_MODE)
      {
        /* Call the corresponding callback to inform upper layer of End of Transfer */
        if (hsmbus->State == HAL_SMBUS_STATE_MASTER_BUSY_TX)
        {
  ...
  else if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_TC) != RESET)
  {
    if (hsmbus->XferCount == 0U)
    {
      ...
      /* Call TxCpltCallback() if no stop mode is set */
      else if (HAL_SMBUS_GET_STOP_MODE(hsmbus) != HAL_SMBUS_AUTOEND_MODE)
      {
        /* No Generate Stop, to permit restart mode */
        /* The stop will be done at the end of transfer, when HAL_SMBUS_AUTOEND_MODE enable */
...
static hal_status_t SMBUS_Slave_ISR(SMBUS_HandleTypeDef *hsmbus, uint32_t StatusFlags)
{
  ...
  if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_AF) != RESET)
  {
    ...
    else
    {
      ...
      SMBUS_Disable_IRQ(hsmbus, HAL_SMBUS_IT_RX | HAL_SMBUS_IT_TX);
  ...
  else if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_ADDR) != RESET)
  {
    TransferDirection = (uint8_t)(HAL_SMBUS_GET_DIR(hsmbus));
    SlaveAddrCode = (uint16_t)(HAL_SMBUS_GET_ADDR_MATCH(hsmbus));

    /* Disable ADDR interrupt to prevent multiple ADDRInterrupt*/
    /* Other ADDRInterrupt will be treat in next Listen usecase */
    __HAL_SMBUS_DISABLE_IT(hsmbus, HAL_SMBUS_IT_ADDRI);
    ...
    HAL_SMBUS_AddrCallback(hsmbus, TransferDirection, SlaveAddrCode);
           ...
           (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_TCR) != RESET))
  {
    if ((hsmbus->State & HAL_SMBUS_STATE_SLAVE_BUSY_RX) == HAL_SMBUS_STATE_SLAVE_BUSY_RX)
    {
      /* Read data from RXDR */
      *hsmbus->pBuffPtr = (uint8_t)(hsmbus->Instance->RXDR);
      ...
      if (hsmbus->XferCount == 1U)
      {
        ...
        hsmbus->XferOptions &= ~HAL_SMBUS_RELOAD_MODE;
        SMBUS_TransferConfig(hsmbus, 0, 1, hsmbus->XferOptions, HAL_SMBUS_NO_STARTSTOP);
      }
  ...
  if (HAL_SMBUS_CHECK_FLAG(StatusFlags, HAL_SMBUS_FLAG_STOPF) != RESET)
  {
    if ((hsmbus->State & HAL_SMBUS_STATE_LISTEN) == HAL_SMBUS_STATE_LISTEN)
    {
      /* Store Last receive data if any */
      if (__HAL_SMBUS_GET_FLAG(hsmbus, HAL_SMBUS_FLAG_RXNE) != RESET)
      {
        /* Read data from RXDR */
        *hsmbus->pBuffPtr = (uint8_t)(hsmbus->Instance->RXDR);
...
static void SMBUS_Enable_IRQ(SMBUS_HandleTypeDef *hsmbus, uint32_t InterruptRequest)
{
  ...
  if ((InterruptRequest & HAL_SMBUS_IT_ALERT) == HAL_SMBUS_IT_ALERT)
  {
    /* Enable ERR interrupt */
    tmpisr |= HAL_SMBUS_IT_ERRI;
  }

  if ((InterruptRequest & HAL_SMBUS_IT_ADDR) == HAL_SMBUS_IT_ADDR)
  {
    /* Enable ADDR, STOP interrupt */
    tmpisr |= HAL_SMBUS_IT_ADDRI | HAL_SMBUS_IT_STOPI | HAL_SMBUS_IT_NACKI | HAL_SMBUS_IT_ERRI;
  }

  if ((InterruptRequest & HAL_SMBUS_IT_TX) == HAL_SMBUS_IT_TX)
  {
    /* Enable ERR, TC, STOP, NACK, RXI interrupt */
    tmpisr |= HAL_SMBUS_IT_ERRI | HAL_SMBUS_IT_TCI | HAL_SMBUS_IT_STOPI | HAL_SMBUS_IT_NACKI | HAL_SMBUS_IT_TXI;
  }

  if ((InterruptRequest & HAL_SMBUS_IT_RX) == HAL_SMBUS_IT_RX)
  {
    /* Enable ERR, TC, STOP, NACK, TXI interrupt */
    tmpisr |= HAL_SMBUS_IT_ERRI | HAL_SMBUS_IT_TCI | HAL_SMBUS_IT_STOPI | HAL_SMBUS_IT_NACKI | HAL_SMBUS_IT_RXI;
  }
...
static void SMBUS_Disable_IRQ(SMBUS_HandleTypeDef *hsmbus, uint32_t InterruptRequest)
{
  ...
  if ((tmpstate == HAL_SMBUS_STATE_READY) && ((InterruptRequest & HAL_SMBUS_IT_ALERT) == HAL_SMBUS_IT_ALERT))
  {
  ...
  if ((InterruptRequest & HAL_SMBUS_IT_TX) == HAL_SMBUS_IT_TX)
  {
    ...
    tmpisr |= HAL_SMBUS_IT_TCI | HAL_SMBUS_IT_TXI;

    if ((HAL_SMBUS_GET_ALERT_ENABLED(hsmbus) == 0UL)
    ...
    if ((tmpstate & HAL_SMBUS_STATE_LISTEN) != HAL_SMBUS_STATE_LISTEN)
    {
      ...
      tmpisr |= HAL_SMBUS_IT_STOPI | HAL_SMBUS_IT_NACKI;
    }
  }
  ...
  if ((InterruptRequest & HAL_SMBUS_IT_RX) == HAL_SMBUS_IT_RX)
  {
    ...
    tmpisr |= HAL_SMBUS_IT_TCI | HAL_SMBUS_IT_RXI;
  ...
  if ((InterruptRequest & HAL_SMBUS_IT_ADDR) == HAL_SMBUS_IT_ADDR)
  {
    ...
    tmpisr |= HAL_SMBUS_IT_ADDRI | HAL_SMBUS_IT_STOPI | HAL_SMBUS_IT_NACKI;
...
static void SMBUS_ITErrorHandler(SMBUS_HandleTypeDef *hsmbus)
{
  ...
  if (((itflags & HAL_SMBUS_FLAG_BERR) == HAL_SMBUS_FLAG_BERR) && \
      ((itsources & HAL_SMBUS_IT_ERRI) == HAL_SMBUS_IT_ERRI))
  {
    hsmbus->ErrorCode |= HAL_SMBUS_ERROR_BERR;

    /* Clear BERR flag */
    __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_BERR);
  }

  /* SMBUS Over-Run/Under-Run interrupt occurred ----------------------------------------*/
  if (((itflags & HAL_SMBUS_FLAG_OVR) == HAL_SMBUS_FLAG_OVR) && \
      ((itsources & HAL_SMBUS_IT_ERRI) == HAL_SMBUS_IT_ERRI))
  {
    hsmbus->ErrorCode |= HAL_SMBUS_ERROR_OVR;

    /* Clear OVR flag */
    __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_OVR);
  }

  /* SMBUS Arbitration Loss error interrupt occurred ------------------------------------*/
  if (((itflags & HAL_SMBUS_FLAG_ARLO) == HAL_SMBUS_FLAG_ARLO) && \
      ((itsources & HAL_SMBUS_IT_ERRI) == HAL_SMBUS_IT_ERRI))
  {
    hsmbus->ErrorCode |= HAL_SMBUS_ERROR_ARLO;

    /* Clear ARLO flag */
    __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_ARLO);
  }

  /* SMBUS Timeout error interrupt occurred ---------------------------------------------*/
  if (((itflags & HAL_SMBUS_FLAG_TIMEOUT) == HAL_SMBUS_FLAG_TIMEOUT) && \
      ((itsources & HAL_SMBUS_IT_ERRI) == HAL_SMBUS_IT_ERRI))
  {
    hsmbus->ErrorCode |= HAL_SMBUS_ERROR_BUSTIMEOUT;

    /* Clear TIMEOUT flag */
    __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_TIMEOUT);
  }

  /* SMBUS Alert error interrupt occurred -----------------------------------------------*/
  if (((itflags & HAL_SMBUS_FLAG_ALERT) == HAL_SMBUS_FLAG_ALERT) && \
      ((itsources & HAL_SMBUS_IT_ERRI) == HAL_SMBUS_IT_ERRI))
  {
  ...
  if (((itflags & HAL_SMBUS_FLAG_PECERR) == HAL_SMBUS_FLAG_PECERR) && \
      ((itsources & HAL_SMBUS_IT_ERRI) == HAL_SMBUS_IT_ERRI))
  {
    hsmbus->ErrorCode |= HAL_SMBUS_ERROR_PECERR;

    /* Clear PEC error flag */
    __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_PECERR);
  }
...
static void SMBUS_Flush_TXDR(SMBUS_HandleTypeDef *hsmbus)
{
  ...
  if (__HAL_SMBUS_GET_FLAG(hsmbus, HAL_SMBUS_FLAG_TXIS) != RESET)
  {
    hsmbus->Instance->TXDR = 0x00U;
  }

  /* Flush TX register if not empty */
  if (__HAL_SMBUS_GET_FLAG(hsmbus, HAL_SMBUS_FLAG_TXE) == RESET)
  {
    __HAL_SMBUS_CLEAR_FLAG(hsmbus, HAL_SMBUS_FLAG_TXE);
  }
}
...
/**
  ...
  *     @arg @ref HAL_SMBUS_RELOAD_MODE Enable Reload mode.
  *     @arg @ref HAL_SMBUS_AUTOEND_MODE Enable Automatic end mode.
  *     @arg @ref HAL_SMBUS_SOFTEND_MODE Enable Software end mode and Reload mode.
  *     @arg @ref HAL_SMBUS_SENDPEC_MODE Enable Packet Error Calculation mode.
  * @param  Request New state of the SMBUS START condition generation.
  *   This parameter can be one of the following values:
  *     @arg @ref HAL_SMBUS_NO_STARTSTOP Don't Generate stop and start condition.
  *     @arg @ref HAL_SMBUS_GENERATE_STOP Generate stop condition (Size should be set to 0).
  *     @arg @ref HAL_SMBUS_GENERATE_START_READ Generate Restart for read request.
  *     @arg @ref HAL_SMBUS_GENERATE_START_WRITE Generate Restart for write request.
  * @retval None
  */
static void SMBUS_TransferConfig(SMBUS_HandleTypeDef *hsmbus,  uint16_t DevAddress, uint8_t Size,
                                 uint32_t Mode, uint32_t Request)
{
  /* Check the parameters */
  ASSERT_DBG_PARAM(IS_SMBUS_ALL_INSTANCE(hsmbus->Instance));
  ASSERT_DBG_PARAM(IS_SMBUS_TRANSFER_MODE(Mode));
  ASSERT_DBG_PARAM(IS_SMBUS_TRANSFER_REQUEST(Request));
...
static void SMBUS_ConvertOtherXferOptions(SMBUS_HandleTypeDef *hsmbus)
{
  /* if user set XferOptions to HAL_SMBUS_OTHER_FRAME_NO_PEC   */
  /* it request implicitly to generate a restart condition */
  /* set XferOptions to HAL_SMBUS_FIRST_FRAME                  */
  if (hsmbus->XferOptions == HAL_SMBUS_OTHER_FRAME_NO_PEC)
  {
    hsmbus->XferOptions = HAL_SMBUS_FIRST_FRAME;
  }
  /* else if user set XferOptions to HAL_SMBUS_OTHER_FRAME_WITH_PEC */
  /* it request implicitly to generate a restart condition      */
  /* set XferOptions to HAL_SMBUS_FIRST_FRAME | HAL_SMBUS_SENDPEC_MODE  */
  else if (hsmbus->XferOptions == HAL_SMBUS_OTHER_FRAME_WITH_PEC)
  {
    hsmbus->XferOptions = HAL_SMBUS_FIRST_FRAME | HAL_SMBUS_SENDPEC_MODE;
  }
  /* else if user set XferOptions to HAL_SMBUS_OTHER_AND_LAST_FRAME_NO_PEC */
  /* it request implicitly to generate a restart condition             */
  /* then generate a stop condition at the end of transfer             */
  /* set XferOptions to HAL_SMBUS_FIRST_AND_LAST_FRAME_NO_PEC              */
  else if (hsmbus->XferOptions == HAL_SMBUS_OTHER_AND_LAST_FRAME_NO_PEC)
  {
    hsmbus->XferOptions = HAL_SMBUS_FIRST_AND_LAST_FRAME_NO_PEC;
  }
  /* else if user set XferOptions to HAL_SMBUS_OTHER_AND_LAST_FRAME_WITH_PEC */
  /* it request implicitly to generate a restart condition               */
  /* then generate a stop condition at the end of transfer               */
  /* set XferOptions to HAL_SMBUS_FIRST_AND_LAST_FRAME_WITH_PEC              */
  else if (hsmbus->XferOptions == HAL_SMBUS_OTHER_AND_LAST_FRAME_WITH_PEC)
  {
    hsmbus->XferOptions = HAL_SMBUS_FIRST_AND_LAST_FRAME_WITH_PEC;
  }
  else
  {
    /* Nothing to do */
