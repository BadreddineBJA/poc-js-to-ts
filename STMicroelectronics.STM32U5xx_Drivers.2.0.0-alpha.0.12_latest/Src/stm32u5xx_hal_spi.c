...
static hal_status_t SPI_WaitOnFlagUntilTimeout(SPI_HandleTypeDef *hspi, uint32_t Flag, FlagStatus FlagStatus,
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Init(SPI_HandleTypeDef *hspi)
{
  uint32_t crc_length;
  ...
  ASSERT_DBG_PARAM(IS_SPI_ALL_INSTANCE(hspi->Instance));
  ASSERT_DBG_PARAM(IS_SPI_MODE(hspi->Init.Mode));
  ASSERT_DBG_PARAM(IS_SPI_DIRECTION(hspi->Init.Direction));
  if (IS_SPI_LIMITED_INSTANCE(hspi->Instance))
  {
    ASSERT_DBG_PARAM(IS_SPI_LIMITED_DATASIZE(hspi->Init.DataSize));
    ASSERT_DBG_PARAM(IS_SPI_LIMITED_FIFOTHRESHOLD(hspi->Init.FifoThreshold));
  }
  else
  {
    ASSERT_DBG_PARAM(IS_SPI_DATASIZE(hspi->Init.DataSize));
    ASSERT_DBG_PARAM(IS_SPI_FIFOTHRESHOLD(hspi->Init.FifoThreshold));
  }
  ASSERT_DBG_PARAM(IS_SPI_NSS(hspi->Init.NSS));
  ASSERT_DBG_PARAM(IS_SPI_NSSP(hspi->Init.NSSPMode));
  ASSERT_DBG_PARAM(IS_SPI_BAUDRATE_PRESCALER(hspi->Init.BaudRatePrescaler));
  ASSERT_DBG_PARAM(IS_SPI_FIRST_BIT(hspi->Init.FirstBit));
  ASSERT_DBG_PARAM(IS_SPI_TIMODE(hspi->Init.TIMode));
  if (hspi->Init.TIMode == SPI_TIMODE_DISABLE)
  {
    ASSERT_DBG_PARAM(IS_SPI_CPOL(hspi->Init.CLKPolarity));
    ASSERT_DBG_PARAM(IS_SPI_CPHA(hspi->Init.CLKPhase));
  }
#if (USE_SPI_CRC != 0UL)
  ASSERT_DBG_PARAM(IS_SPI_CRC_CALCULATION(hspi->Init.CRCCalculation));
  if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE)
  {
    if (IS_SPI_LIMITED_INSTANCE(hspi->Instance))
    {
      ASSERT_DBG_PARAM(IS_SPI_LIMITED_CRC_LENGTH(hspi->Init.CRCLength));
    }
    else
    {
      ASSERT_DBG_PARAM(IS_SPI_CRC_LENGTH(hspi->Init.CRCLength));
    }
    ASSERT_DBG_PARAM(IS_SPI_CRC_POLYNOMIAL(hspi->Init.CRCPolynomial));
    ASSERT_DBG_PARAM(IS_SPI_CRC_INITIALIZATION_PATTERN(hspi->Init.TxCRCInitializationPattern));
    ASSERT_DBG_PARAM(IS_SPI_CRC_INITIALIZATION_PATTERN(hspi->Init.RxCRCInitializationPattern));
  }
#else
  hspi->Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;
#endif /* USE_SPI_CRC */

  ASSERT_DBG_PARAM(IS_SPI_RDY_MASTER_MANAGEMENT(hspi->Init.ReadyMasterManagement));
  ASSERT_DBG_PARAM(IS_SPI_RDY_POLARITY(hspi->Init.ReadyPolarity));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_DeInit(SPI_HandleTypeDef *hspi)
{
  /* Check the SPI handle allocation */
  if (hspi == NULL)
  {
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_RegisterCallback(SPI_HandleTypeDef *hspi, HAL_SPI_CallbackIDTypeDef CallbackID,
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_UnRegisterCallback(SPI_HandleTypeDef *hspi, HAL_SPI_CallbackIDTypeDef CallbackID)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Transmit(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
#if defined (__GNUC__)
  ...
  uint32_t tickstart;
  hal_status_t errorcode = HAL_OK;

  /* Check Direction parameter */
  ASSERT_DBG_PARAM(IS_SPI_DIRECTION_2LINES_OR_1LINE_2LINES_TXONLY(hspi->Init.Direction));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Receive(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size, uint32_t Timeout)
{
  uint32_t tickstart;
  hal_status_t errorcode = HAL_OK;
#if defined (__GNUC__)
  ...
  /* Check Direction parameter */
  ASSERT_DBG_PARAM(IS_SPI_DIRECTION_2LINES_OR_1LINE_2LINES_RXONLY(hspi->Init.Direction));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_TransmitReceive(SPI_HandleTypeDef *hspi, uint8_t *pTxData, uint8_t *pRxData, uint16_t Size,
                                     uint32_t Timeout)
{
  ...
  /* Check Direction parameter */
  ASSERT_DBG_PARAM(IS_SPI_DIRECTION_2LINES(hspi->Init.Direction));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Transmit_IT(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size)
{
  hal_status_t errorcode = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Receive_IT(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size)
{
  hal_status_t errorcode = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_TransmitReceive_IT(SPI_HandleTypeDef *hspi, uint8_t *pTxData, uint8_t *pRxData, uint16_t Size)
{
  HAL_SPI_StateTypeDef  tmp_state;
  hal_status_t errorcode = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Transmit_DMA(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size)
{
  hal_status_t errorcode;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Receive_DMA(SPI_HandleTypeDef *hspi, uint8_t *pData, uint16_t Size)
{
  hal_status_t errorcode;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_TransmitReceive_DMA(SPI_HandleTypeDef *hspi, uint8_t *pTxData, uint8_t *pRxData,
                                         uint16_t Size)
{
  HAL_SPI_StateTypeDef tmp_state;
  hal_status_t errorcode;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Abort(SPI_HandleTypeDef *hspi)
{
  hal_status_t errorcode;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_SPI_Abort_IT(SPI_HandleTypeDef *hspi)
{
  hal_status_t errorcode;
  __IO uint32_t count;
...
/**
  ...
  * @retval HAL_ERROR
  */
hal_status_t HAL_SPI_DMAPause(SPI_HandleTypeDef *hspi)
{
...
/**
  ...
  * @retval HAL_ERROR
  */
hal_status_t HAL_SPI_DMAResume(SPI_HandleTypeDef *hspi)
{
  /* Set error code to not supported */
  SET_BIT(hspi->ErrorCode, HAL_SPI_ERROR_NOT_SUPPORTED);
...
/**
  ...
  * @retval HAL_ERROR
  */
hal_status_t HAL_SPI_DMAStop(SPI_HandleTypeDef *hspi)
{
  /* Set error code to not supported */
  SET_BIT(hspi->ErrorCode, HAL_SPI_ERROR_NOT_SUPPORTED);
...
/**
  ...
  * @retval HAL status
  */
static hal_status_t SPI_WaitOnFlagUntilTimeout(SPI_HandleTypeDef *hspi, uint32_t Flag, FlagStatus Status,
