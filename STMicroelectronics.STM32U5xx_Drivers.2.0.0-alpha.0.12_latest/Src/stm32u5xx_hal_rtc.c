...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_Init(RTC_HandleTypeDef *hrtc)
{
  ...
  if (hrtc != NULL)
  {
    ...
    ASSERT_DBG_PARAM(IS_RTC_ALL_INSTANCE(hrtc->Instance));
    ASSERT_DBG_PARAM(IS_RTC_HOUR_FORMAT(hrtc->Init.HourFormat));
    ASSERT_DBG_PARAM(IS_RTC_ASYNCH_PREDIV(hrtc->Init.AsynchPrediv));
    ASSERT_DBG_PARAM(IS_RTC_SYNCH_PREDIV(hrtc->Init.SynchPrediv));
    ASSERT_DBG_PARAM(IS_RTC_OUTPUT(hrtc->Init.OutPut));
    ASSERT_DBG_PARAM(IS_RTC_OUTPUT_REMAP(hrtc->Init.OutPutRemap));
    ASSERT_DBG_PARAM(IS_RTC_OUTPUT_POL(hrtc->Init.OutPutPolarity));
    ASSERT_DBG_PARAM(IS_RTC_OUTPUT_TYPE(hrtc->Init.OutPutType));
    ASSERT_DBG_PARAM(IS_RTC_OUTPUT_PULLUP(hrtc->Init.OutPutPullUp));
    ASSERT_DBG_PARAM(IS_RTC_BINARY_MODE(hrtc->Init.BinMode));
    ASSERT_DBG_PARAM(IS_RTC_BINARY_MIX_BCDU(hrtc->Init.BinMixBcdU));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_DeInit(RTC_HandleTypeDef *hrtc)
{
  hal_status_t status;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_RegisterCallback(RTC_HandleTypeDef *hrtc, HAL_RTC_CallbackIDTypeDef CallbackID,
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_UnRegisterCallback(RTC_HandleTypeDef *hrtc, HAL_RTC_CallbackIDTypeDef CallbackID)
{
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_SetTime(RTC_HandleTypeDef *hrtc, RTC_TimeTypeDef *sTime, uint32_t Format)
{
  uint32_t tmpreg;
  hal_status_t status;
  ...
  if (status == HAL_OK)
  {
    ...
    if (READ_BIT(RTC->ICSR, RTC_ICSR_BIN) != RTC_BINARY_ONLY)
    {
      if (Format == RTC_FORMAT_BIN)
      {
        ...
        {
          ASSERT_DBG_PARAM(IS_RTC_HOUR12(sTime->Hours));
          ASSERT_DBG_PARAM(IS_RTC_HOURFORMAT12(sTime->TimeFormat));
        }
        else
        {
          sTime->TimeFormat = 0x00U;
          ASSERT_DBG_PARAM(IS_RTC_HOUR24(sTime->Hours));
        }
        ASSERT_DBG_PARAM(IS_RTC_MINUTES(sTime->Minutes));
        ASSERT_DBG_PARAM(IS_RTC_SECONDS(sTime->Seconds));
      ...
      else
      {
        if (READ_BIT(RTC->CR, RTC_CR_FMT) != 0U)
        {
          ASSERT_DBG_PARAM(IS_RTC_HOUR12(RTC_Bcd2ToByte(sTime->Hours)));
          ASSERT_DBG_PARAM(IS_RTC_HOURFORMAT12(sTime->TimeFormat));
        }
        else
        {
          sTime->TimeFormat = 0x00U;
          ASSERT_DBG_PARAM(IS_RTC_HOUR24(RTC_Bcd2ToByte(sTime->Hours)));
        }
        ASSERT_DBG_PARAM(IS_RTC_MINUTES(RTC_Bcd2ToByte(sTime->Minutes)));
        ASSERT_DBG_PARAM(IS_RTC_SECONDS(RTC_Bcd2ToByte(sTime->Seconds)));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_GetTime(RTC_HandleTypeDef *hrtc, RTC_TimeTypeDef *sTime, uint32_t Format)
{
  uint32_t tmpreg;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_SetDate(RTC_HandleTypeDef *hrtc, RTC_DateTypeDef *sDate, uint32_t Format)
{
  uint32_t datetmpreg;
  hal_status_t status;
  ...
  ASSERT_DBG_PARAM(IS_RTC_WEEKDAY(sDate->WeekDay));

  if (Format == RTC_FORMAT_BIN)
  {
    ASSERT_DBG_PARAM(IS_RTC_YEAR(sDate->Year));
    ASSERT_DBG_PARAM(IS_RTC_MONTH(sDate->Month));
    ASSERT_DBG_PARAM(IS_RTC_DATE(sDate->Date));
  ...
  {
    ASSERT_DBG_PARAM(IS_RTC_YEAR(RTC_Bcd2ToByte(sDate->Year)));
    ASSERT_DBG_PARAM(IS_RTC_MONTH(RTC_Bcd2ToByte(sDate->Month)));
    ASSERT_DBG_PARAM(IS_RTC_DATE(RTC_Bcd2ToByte(sDate->Date)));
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_GetDate(RTC_HandleTypeDef *hrtc, RTC_DateTypeDef *sDate, uint32_t Format)
{
  uint32_t datetmpreg;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_SetAlarm(RTC_HandleTypeDef *hrtc, RTC_AlarmTypeDef *sAlarm, uint32_t Format)
{
  uint32_t tmpreg = 0;
  uint32_t binaryMode;
  ...
  {
    ASSERT_DBG_PARAM(IS_RTC_FORMAT(Format));
    ASSERT_DBG_PARAM(IS_RTC_ALARM(sAlarm->Alarm));
    ASSERT_DBG_PARAM(IS_RTC_ALARM_MASK(sAlarm->AlarmMask));
    ASSERT_DBG_PARAM(IS_RTC_ALARM_DATE_WEEKDAY_SEL(sAlarm->AlarmDateWeekDaySel));
    ASSERT_DBG_PARAM(IS_RTC_ALARM_SUB_SECOND_VALUE(sAlarm->AlarmTime.SubSeconds));
    ASSERT_DBG_PARAM(IS_RTC_ALARM_SUB_SECOND_MASK(sAlarm->AlarmSubSecondMask));
  }
  else if (READ_BIT(RTC->ICSR, RTC_ICSR_BIN) == RTC_BINARY_ONLY)
  {
    ASSERT_DBG_PARAM(IS_RTC_ALARM_SUB_SECOND_BINARY_MASK(sAlarm->AlarmSubSecondMask));
    ASSERT_DBG_PARAM(IS_RTC_ALARMSUBSECONDBIN_AUTOCLR(sAlarm->BinaryAutoClr));
  }
  else /* RTC_BINARY_MIX */
  {
    ASSERT_DBG_PARAM(IS_RTC_FORMAT(Format));
    ...
    /* In Binary Mix Mode, the RTC can not generate an alarm on a match involving all calendar items
       + the upper SSR bits */
    ASSERT_DBG_PARAM((sAlarm->AlarmSubSecondMask >> RTC_ALRMASSR_MASKSS_Pos) <=
  ...
  if (binaryMode != RTC_BINARY_ONLY)
  {
    if (Format == RTC_FORMAT_BIN)
    {
      if (READ_BIT(RTC->CR, RTC_CR_FMT) != 0U)
      {
        ASSERT_DBG_PARAM(IS_RTC_HOUR12(sAlarm->AlarmTime.Hours));
        ASSERT_DBG_PARAM(IS_RTC_HOURFORMAT12(sAlarm->AlarmTime.TimeFormat));
      }
      else
      {
        sAlarm->AlarmTime.TimeFormat = 0x00U;
        ASSERT_DBG_PARAM(IS_RTC_HOUR24(sAlarm->AlarmTime.Hours));
      }
      ASSERT_DBG_PARAM(IS_RTC_MINUTES(sAlarm->AlarmTime.Minutes));
      ASSERT_DBG_PARAM(IS_RTC_SECONDS(sAlarm->AlarmTime.Seconds));

      if (sAlarm->AlarmDateWeekDaySel == RTC_ALARMDATEWEEKDAYSEL_DATE)
      {
        ASSERT_DBG_PARAM(IS_RTC_ALARM_DATE_WEEKDAY_DATE(sAlarm->AlarmDateWeekDay));
      }
      else
      {
        ASSERT_DBG_PARAM(IS_RTC_ALARM_DATE_WEEKDAY_WEEKDAY(sAlarm->AlarmDateWeekDay));
      }
    ...
    else /* format BCD */
    {
      if (READ_BIT(RTC->CR, RTC_CR_FMT) != 0U)
      {
        ASSERT_DBG_PARAM(IS_RTC_HOUR12(RTC_Bcd2ToByte(sAlarm->AlarmTime.Hours)));
        ASSERT_DBG_PARAM(IS_RTC_HOURFORMAT12(sAlarm->AlarmTime.TimeFormat));
      }
      else
      {
        sAlarm->AlarmTime.TimeFormat = 0x00U;
        ASSERT_DBG_PARAM(IS_RTC_HOUR24(RTC_Bcd2ToByte(sAlarm->AlarmTime.Hours)));
      }

      ASSERT_DBG_PARAM(IS_RTC_MINUTES(RTC_Bcd2ToByte(sAlarm->AlarmTime.Minutes)));
      ASSERT_DBG_PARAM(IS_RTC_SECONDS(RTC_Bcd2ToByte(sAlarm->AlarmTime.Seconds)));

#ifdef  USE_FULL_ASSERT
      if (sAlarm->AlarmDateWeekDaySel == RTC_ALARMDATEWEEKDAYSEL_DATE)
      {
        ASSERT_DBG_PARAM(IS_RTC_ALARM_DATE_WEEKDAY_DATE(RTC_Bcd2ToByte(sAlarm->AlarmDateWeekDay)));
      }
      else
      {
        ASSERT_DBG_PARAM(IS_RTC_ALARM_DATE_WEEKDAY_WEEKDAY(RTC_Bcd2ToByte(sAlarm->AlarmDateWeekDay)));
      }
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_SetAlarm_IT(RTC_HandleTypeDef *hrtc, RTC_AlarmTypeDef *sAlarm, uint32_t Format)
{
  uint32_t tmpreg = 0;
  uint32_t binaryMode;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_DeactivateAlarm(RTC_HandleTypeDef *hrtc, uint32_t Alarm)
{
  /* Check the parameters */
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_GetAlarm(RTC_HandleTypeDef *hrtc, RTC_AlarmTypeDef *sAlarm, uint32_t Alarm, uint32_t Format)
{
  uint32_t tmpreg;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_PollForAlarmAEvent(RTC_HandleTypeDef *hrtc, uint32_t Timeout)
{
...
/**
  ...
  * @retval HAL status
  */
hal_status_t HAL_RTC_WaitForSynchro(RTC_HandleTypeDef *hrtc)
{
  uint32_t tickstart;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t RTC_EnterInitMode(RTC_HandleTypeDef *hrtc)
{
  uint32_t tickstart;
  hal_status_t status = HAL_OK;
...
/**
  ...
  * @retval HAL status
  */
hal_status_t RTC_ExitInitMode(RTC_HandleTypeDef *hrtc)
{
  hal_status_t status = HAL_OK;
