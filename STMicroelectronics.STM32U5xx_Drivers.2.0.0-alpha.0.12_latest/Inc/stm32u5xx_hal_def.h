...
typedef enum
{
  HAL_OK            = 0x00, /* HAL operation completed successfully */
  HAL_ERROR         = 0x01, /* HAL operation completed with error   */
  HAL_BUSY          = 0x02, /* HAL concurrent process ongoing       */
  HAL_INVALID_PARAM = 0x03, /* HAL invalid parameter                */
  HAL_TIMEOUT       = 0x04  /* HAL operation exceeds user timeout   */
...
/** @brief Check the current peripheral handle state and move it to new state in an atomic way.
  ...
  * @param __PPP_STATE_FIELD__ specifies the state field within the Handle.
  * @param __PPP_CONDITIONAL_STATE__ state to be checked to authorize moving to the new state.
  * @param __PPP_NEW_STATE__ new state to be set.
  ...
  *         __PPP_CONDITIONAL_STATE__.
  */
#if defined(USE_HAL_CHECK_PROCESS_STATE) && (USE_HAL_CHECK_PROCESS_STATE == 1U)
#define HAL_CHECK_UPDATE_STATE(__HANDLE__, __PPP_STATE_FIELD__, __PPP_CONDITIONAL_STATE__, __PPP_NEW_STATE__)         \
  do {                                                                                                                \
      do{                                                                                                             \
...
#else
#define HAL_CHECK_UPDATE_STATE(__HANDLE__, __PPP_STATE_FIELD__, __PPP_CONDITIONAL_STATE__, __PPP_NEW_STATE__) \
  (__HANDLE__)->__PPP_STATE_FIELD__ = (__PPP_NEW_STATE__)
